...
### 2026-09-01
- **[更新] 書き出しパイプラインは1コマンドの手動起動でなく、Kana の HTML コミットを起点に自動起動＋差分のみ再変換にする（旧 2026-08-18 を更新）**：「媒体別サイズ生成→AVIF併産→35%/50%縮小版→容量・naturalWidth検証→配信面モック合成（2026-08-27参照）」まで工程が伸びた結果、1コマンドでも全件実行は重く、ローテーション3本×7社×媒体別サイズで数十枚を毎回焼き直すことになる。HTML・`brand-tokens/{client}.json`・`compression-profile.json` の内容ハッシュを出力のキャッシュキーにし、変更のあった組み合わせだけ再変換する差分ビルドへ変える。手動起動を待たずコミット起点で走るため、Kana の修正から検証済み納品物が揃うまでのHiro待ち時間そのものが消える
...
### 2026-09-02
- **[更新] `preparePage()` の待機は読込完了までで止めず、アニメーションを初期状態＝最終状態へ固定するところまで含める（旧 2026-08-05 を更新）**：`document.fonts.ready`＋`getAnimations()` 全 finished＋背景プリロード＋`<img>` naturalWidth 検証を一本化しても、`IntersectionObserver` 発火のフェードインや `transition-delay` 付きの遅延実行は未発火のまま撮られ、要素が `opacity:0` で消えた PNG が出る。`page.emulateMediaFeatures([{name:'prefers-reduced-motion', value:'reduce'}])` で初期状態を最終状態へ寄せたうえで `getAnimations().forEach(a=>a.finish())` を強制実行してから撮影し、出力の非空ピクセル比が基準スナップショット（2026-09-01参照）を下回った枚は自動で再撮に回す。消えた要素を目視でなく数値で検出する
...
### 2026-09-09
- （よくある失敗）クライアントのロゴがSVGでなくPDFのみで支給され、Puppeteerのヘッドレス環境でPDF埋め込み画像をそのままラスタライズした結果、ベクター特有の輪郭がジャギー化してPNG化後に「ロゴがぼやけている」と差し戻される。回避策：SVG/PDFロゴは変換前に`sharp`または`resvg`で高解像度ラスタ化（目標表示幅の3倍以上）を一度挟んでからHTMLへ埋め込み、Puppeteerのビューポート拡大に依存したラスタライズをさせない
