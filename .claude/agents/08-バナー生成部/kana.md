...
### 2026-07-11
- **書体分類の用語（セリフ/サンセリフ/明朝/ゴシック/丸ゴシック）を素材選定の共通語に**：セリフ＝欧文のうろこ付き（≒和文の明朝、格調・高級）、サンセリフ＝うろこなし（≒和文のゴシック、視認・力強さ）、丸ゴシック＝角を丸めたゴシック（親しみ・柔らかさ、主婦・介護系採用に有効）。和欧混植では「明朝＋セリフ」「ゴシック＋サンセリフ」で分類を揃えるのが原則で、明朝見出しに欧文サンセリフを混ぜると数字だけ浮く。業種トーンに丸ゴシックを足す判断も分類名で Rei/Yuna と共有する
- **ウェイト（font-weight）とオプティカルサイズの用語を Google Fonts 指定で正しく扱う**：ウェイト＝線の太さ（100〜900、Noto Sans JP は 400/500/700/900 等の飛び飛びで、link の `wght@` に列挙した値しか出ない）、オプティカルサイズ＝表示サイズに応じて字形を最適化する可変軸（`opsz`、小サイズは字間広め・大サイズは締まった字形）。`font-weight:600` を指定しても link に 600 が無ければ最寄りにフォールバックして意図と変わるため、使用ウェイトは link href に必ず全列挙する
...
### 2026-07-16
- **Hiro から差し戻された PNG の欠陥は「Kana のローカルプレビューでも再現するか」を確認してから直す連携**：再現しない＝環境差起因（フォントレンダリング差・OpenType フィーチャ適用差・CSS 背景読込差）で、HTML 側を触ると Kana 環境の見た目まで壊れる。再現しない欠陥は Hiro の `preparePage` 側で吸収するのが正しい持ち場のため、切り分け結果を添えて返す。pixelmatch の差分ヒートマップがあれば再現確認は1分で済む
...
### 2026-07-27
- **CSS `text-wrap: pretty` が主要ブラウザに広がり、`balance` との使い分けが2026年の字組みトレンドに**：`balance`は見出しの行長を均等化する用途、`pretty`は本文向けで最終行の孤立（ウィドウ）や不格好な折返しだけを抑える最適化。バナーのメインコピーは`balance`、複数行のサブコピー・注釈は`pretty`と役割で使い分けると、これまで`nowrap`で手当てしていた泣き別れをブラウザ側が自動処理してくれる範囲が増える
...
### 2026-08-03
- **CSS `text-box`/`text-box-trim`（行の上下ハーフレディング除去）が縦位置揃えの新定石に**：フォント固有の上下余白をトリムして、大きい数字と単位・アイコンの視覚的な天地中央を正確に合わせられる。「月給35万」の数字と「万円」のベースライン沈み補正を手動`translateY`でなく標準プロパティで実現でき、CTAボタン内テキストの上下センタリングも安定。Hiroの決定性チェック（同一HTML2回変換でピクセル一致）とも相性が良い
...
### 2026-08-05
- （よくある失敗）`vw`でフォントサイズを指定し、Hiroの解像度目的のビューポート拡大で文字が肥大化して崩れる。回避策：`cqw`＋`clamp()`でバナーキャンバス幅に連動させ、サイズ違い展開の文字スケール破綻を防ぐ
...
### 2026-08-12
- （よくある失敗）バナー内で`position:fixed`/`sticky`や`vh`依存レイアウトを使い、Hiroの静止キャプチャで位置が想定とズレる。回避策：バナーは静止画完結が前提のためfixed/sticky/vh依存を禁止し、絶対配置は必ずバナー親要素基準（`position:relative`親＋`absolute`子）に閉じる
- （よくある失敗）`background-image`を相対パスで指定し、ヘッドレス環境でパス解決できず背景抜けのまま書き出し。回避策：画像は絶対パスかbase64インライン化で埋め込み、`document.fonts.ready`と同様に画像のプリロード完了を保証する構造にしてHiroへ渡す
- （よくある失敗）clip境界ぎりぎりに影・はみ出し装飾・角丸の外側を置き、Hiroのclipで見切れて欠ける。回避策：影や装飾も含めた要素を媒体セーフエリア内に収め、意図的にトリムしたい外側だけを`overflow:hidden`で切る設計にする
- （よくある失敗）Google Fontsを`@import`やHTML末尾の`<link>`で読み、FOUT（未適用）状態のままキャプチャされフォールバック体で焼き込まれる。回避策：`<link rel="preload">`＋`<head>`先頭配置で先読みし、Hiroの`fonts.ready`待ちと二重で未適用を防ぐ

### 2026-08-13
- **07-LP 部 iro との「用途タグ付きパレット直流用」連携**：LP↔バナー世界観統一案件では、iro が抽出した用途タグ付きパレット（`--primary`/`--secondary`/`--accent`/`--text`）を tsumugi・Yuna 経由で受け取り、自分の `:root` へ一字一句コピーする。近似色を自分で作ると広告→LP で微妙な色ズレが出て「別ブランド感」を招くため、HEX は推測せず iro の確定 `design-tokens.json` を唯一ソースにする。色の当て違いによる Yuna 差し戻しも構造的に消える（理由：ブランド色は抽出者が単一ソース、複製側が近似すると必ずズレる）
...
### 2026-08-16
- **[更新] 「最小可読サイズ」はキャンバス基準でなく、求職者が実際に見る表示幅から逆算して決める（旧 2026-07-21 を更新）**：1080×1080 のバナーは Indeed の求人一覧や SNS フィードで幅 320〜400px＝約30〜37% に縮小表示されるため、キャンバス上 24px の文字は実機で約 7〜9px となり判読不能になる。実表示で 11px 以上を確保する＝1080px キャンバスなら約 30px 以上、給与などの主訴求は 40px 以上を下限としてルール化する。「小サイズ展開で潰れる」という旧ルールの目的は、展開サイズでなく実配信の縮小率を基準に据えて初めて満たせる
...
### 2026-08-18
- レイアウトは白紙から組まず、条件3点（給与・職種・勤務地）の面積配分と最小pxを規定済みのグリッドテンプレから選ぶ。配分の議論が毎回発生せず、実表示11px以上の下限も自動的に満たされる
...
### 2026-08-27
- **[更新] ラフ段階の 35% 縮小確認は自分でズームせず、ラフ HTML を Hiro の縮小版生成パイプラインへ投げて返してもらう（旧 2026-08-18 を更新）**：Hiro は「媒体別サイズ生成→AVIF併産→35%/50% 縮小版→容量・naturalWidth 検証」を1コマンドで回している。ブラウザのズームで縮めた見え方は実際の書き出しとアンチエイリアスのかかり方が違い、細い縁取りやバッジ文字の潰れ判定がズレる。ラフ時点の HTML を「縮小版だけ欲しい」タグ付きで渡す軽量レーンを Hiro と作れば、判定が実出力ベースになり、組み替えが数分で済むラフ段階のうちに構成の可否が確定する
...
### 2026-09-01
- **全サイズ・全色パターンを1ページに並べた確認用インデックスHTMLを案件フォルダに1枚置き、ブラウザ1枚で俯瞰する**：`iframe` か `transform: scale()` で各バナーHTMLをグリッド表示し、実表示相当（35%前後）の縮尺で並べる。ファイルを1つずつ開いて確認すると、サイズ間のロゴ位置・面積配分のズレや1案だけ色が古いといった横並びで初めて見える崩れを取りこぼす。Yuna への社内提示も同じURLで済み、Hiro の書き出しを待たずにラフ段階の可否が確定する
...
### 2026-09-02
- （よくある失敗）写真素材の上に白文字を直接置き、空・ヘルメットの反射・白い車両など明るい領域に重なった箇所だけ文字が消える。原因はコントラスト比を写真の平均色や最暗部で測って合格にしていること。回避策は写真とテキストの間に `linear-gradient(rgba(0,0,0,.55), rgba(0,0,0,0))` のスクリム帯を1枚挟み、コントラストは文字が乗る領域の最明部で測る。`text-shadow`／`-webkit-text-stroke` での可読化は35%縮小（2026-08-27参照）で滲んで潰れるため代替にしない
...
### 2026-09-09
- （よくある失敗）ダークモード配信面（一部SNSアプリのダークテーマフィード）を想定せず白背景バナーを組み、白フィード確認だけで校了し、実際のダーク配信面ではロゴの黒文字部分が背景に溶けて社名が読めない状態で入稿される。回避策：Hiroの白/黒2種背景合成確認（2026-08-27参照）を校了時の必須添付にし、ロゴ・テキストの単色使用箇所は`brand-tokens`の`--text`が両背景で判読可能な明度かを事前に確認してから配色を決める
