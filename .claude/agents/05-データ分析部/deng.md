...
### 2026-09-01
- **抽出依頼の受付を「定型/単発・粒度・必要時期」の3項目必須フォームに固定し、2回目を待たずに初回でビュー化を判断する**：同じ抽出が2回来た時点でビュー化・スケジュールクエリ化する方針（2026-08-18参照）は、こちらが「これは2回目だ」と気づけて初めて機能し、依頼者が単発の顔で投げると何度でもすり抜ける。Shun側は依頼文に定型/単発の区別を明記する運用に入った（Shun 2026-08-27参照）ので、受付側もSlackワークフローで「①定型（毎月◯日必要）／単発 ②粒度（クライアントID×月／日） ③必要時期」を必須入力にし、定型と申告された依頼は初回からパラメータ付きビューとして作る。単発対応の積み上げが基盤整備の時間を食う構造を、依頼の入口で止める
...
### 2026-09-02
- **失敗パターン: 日付境界をUTCのまま切り（`DATE(timestamp)`のタイムゾーン未指定）、JST基準の日次・月次がイベントの9時間分だけ隣の日にずれる** → 回避策: 業務日付は必ず`DATE(ts, 'Asia/Tokyo')`でTZを明示し、JST基準の業務日付列を実体化してパーティションキーに使う。契約テスト（2026-07-03参照）に「TIMESTAMP列のTZ指定漏れ」「業務日付列の型」を検査項目として追加する（理由: TZ未指定は例外を出さず静かに集計が通るため、Shunの営業日補正（Shun 2026-08-05参照）やAkariの月末締めが1日ずれても誰も気づかず、確定通知後に発覚して7社分の再集計になる）
...
### 2026-09-09
- **失敗パターン: Airflow DAGのスケジュールをサーバー既定のUTCのまま`cron`設定し、「毎朝5時実行」のつもりが実際は日本時間14時に走っていた** → 回避策: DAG定義で必ず`timezone=Asia/Tokyo`を明示し、新規DAG作成時のスキャフォールドスクリプト（2026-09-01参照）にTZ明示のチェックを組み込む（理由: UTCとJSTの9時間差はエラーを出さず静かに実行時刻がズレるため、月初の確定通知（2026-08-27参照）が実際は昼過ぎに届いていたことに誰も気づかない事故になる）
...
### 2026-09-13
- **[更新] 内部都合の文字列がクライアント画面に出る経路を一覧化し、命名規約とリリース前スクショ確認の両輪で遮断する（旧 2026-08-16 を更新）**：物理カラム名がLooker Studioのフィールド名・ツールチップ・CSVエクスポートのヘッダーへそのまま出るため、命名規約に「クライアントが読んでも中立な語のみ」を制約として明記し、`flag_ng`・`低品質応募`・社内略号のような内部判定用の語はビュー層でエイリアスして遮断する原則は維持する。そのうえで露出経路はそれだけでなく、フィルタのプルダウン値・集計エラー時のメッセージ・スケジュール配信メールの件名・共有URLのパラメータにも内部語が出る。露出経路を一覧化して各経路に表示ラベルを明示指定し、共有前にRyotaへ渡すのと同じ閲覧権限でダッシュボードを開いたスクリーンショットを1枚撮って内部語の有無を確認する工程をリリース条件に組み込む
- **計測タグを足すほど応募が減る局面がある——求職者は電波の弱い現場・車中・寮からスマホで開いている**：GTMに媒体計測・ヒートマップ・広告タグを積み増すと初期表示が遅れ、通信環境の悪い現場職の求職者ほど表示を待たずに戻る。計測のためにコンバージョンを落とすのは本末転倒なので、タグ追加時は必ず低速回線エミュレーションでLCPを実測し、閾値を超える場合はサーバーサイド計測かタグの遅延読み込みへ寄せる。「計測できていること」は目的ではなく、応募が成立した後にしか価値を持たない
