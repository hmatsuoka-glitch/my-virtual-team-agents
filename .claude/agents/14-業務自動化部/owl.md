...
### 2026-08-27
- **[更新] Sales連携：ヨミ段階の案件は「確定前は前進させない」で止めず、仮引当ステートを正常系に用意して受ける（旧 2026-08-13 を更新）**：商談ステージ（ヨミA=内示／確定）とOrderConfirmedを1対1で対応づける合意は前提としつつ、実務ではヨミAで先に発注・在庫を押さえたい要望が必ず混ざる。これを異常系の手動運用に押し出すと現場が状態機械の外で動き出すため、「仮引当」を正常系ステートとして置き、確定で本引当へ、失注でReservationReleasedへ抜ける補償経路（06-11記録）を最初から張る。Boのhandoff配布（Bo 07-16記録）へは、仮引当段階では配布せず確定遷移でのみ発火する条件を渡す。
- **Bo連携：状態遷移表には遷移ごとに「自動打刻／人手操作／人手＋承認」の3区分を列として明示して渡す**：Boは可逆性で全自動と承認関門を切り分ける（Bo 07-16記録）が、その判断材料はOwl側の設計にしかない。現場で1タップの遷移（着工・搬入・完了／08-18記録）は自動打刻で冪等に、ピボット地点（06-20記録）を越える遷移は承認関門付き、という対応を1表で示すとBoが遷移ごとに実装方針を問い合わせる往復が消える。同じ表にAdminOverride（08-12記録）の実行権限も併記する。
- **Kpi連携：経営向けの金額換算（08-16記録）はOwl側で係数を持たず、換算式をKpi経由でFinance確定値に揃える**：「承認待ち平均◯日＝立替金の寝かせ日数◯円」の日あたり資金コスト・再訪コストをOwlが独自に置くと、同じ滞留日数が受注フロー報告と経営ダッシュボードで別金額になる。k4（06-11記録）を送る際は滞留日数の実数だけを渡し、金額換算はKpi側の単一lookupで当ててもらう役割分担にする。SLA違反の発火／解消イベントを両方送る（07-16記録）のと同じく、判定と換算の責任をKpi側へ寄せる。
- **Dat連携：設計外経路の頻度実測（08-05記録）を依頼するときは、経路別の件数を分子・分母の実数で渡す**：Datは比率を加重平均で出す方針（Dat 08-12記録）のため、Owl側で「全遷移に占める設計外経路の割合◯%」に丸めて渡すと再集計できず、案件数の少ないクライアントの経路が過大に効いた数字で正常系昇格を判断してしまう。依頼文には「効果検証でなく頻度の実測が目的」（07-16記録）と明記し、月次件数だけを軽く返してもらう。

### 2026-09-01
- **遷移表は図から起こさず、標準遷移モデル（受注→着工→搬入→完了→請求確定・08-18記録）のCSVに必要列を最初から持たせて「セルを埋めるだけ」にする**：列は①遷移ごとの3区分（自動打刻／人手／人手＋承認・08-27記録）②現場1タップか事務所か（08-16記録）③補償イベントとロールバックSQL（06-11記録）④実行可能ロール（07-03記録）⑤ピボット地点か（06-20記録）。PlantUML図もBo実装即着手パッケージ（07-07記録）もこのCSVから生成し、図と表を二重管理しない。列が空のまま設計レビューへ出せない構造にすると、引き渡し後にBoから遷移ごとの実装方針を問い合わせられる往復（08-27記録）が消える。
...
### 2026-09-02
- （よくある失敗）イベントのフィールド名や意味を後から変更しながら schema_version を持たせておらず、過去イベントのリプレイで状態が再現できなくなる → 回避策：全イベントに schema_version を必須で持たせ、変更は追加のみ（意味が変わる場合は新イベント型＋旧版のアップキャスタ）を原則にする（理由：イベントソーシングの唯一の真実・06-13記録は「過去のイベントが今のコードで解釈できること」が前提で、破壊的変更は過去時点復元・07-03記録と AdminOverride の証跡・08-12記録をまとめて無効化する）
...
### 2026-09-09
- **失敗パターン: 現場の1タップ遷移（08-16/08-18記録）の画面ラベルを日本語固定にしており、外国人技能実習生・特定技能人材が担当する現場で誤操作（完了ボタンと搬入ボタンの押し間違い）が発生し状態が実態とズレる** → 回避策: 現場向け画面ラベルを多言語対応（やさしい日本語＋ピクトグラム併記）にし、誤操作からの取り消し導線を1タップ遷移側にも用意する。取り消し不能な遷移（05-24記録）の心理ハードルはピクトグラムでも変わらないため、復帰導線を優先する。
...
### 2026-09-13
- **協力会社・職人視点：自社システムに入らない相手が起点の遷移は、必ず又聞き入力になり時刻が信用できない**：搬入完了・出面（人工）報告は協力会社側で発生するのに、遷移の実行主体を自社社員に設計すると、電話で聞いた内容を事務員が後から打つ形になり、実時刻の自動打刻（08-16記録）が成立しない。社外向けにはログイン不要のワンタイムURL1画面（1タップ＋写真1枚）を用意し、遷移表の「実行可能ロール」列（09-01記録）に社外ロールを明示する。アカウント発行を前提にした設計は、協力会社の入れ替わりが多い建設案件では初月で運用から外れる。
