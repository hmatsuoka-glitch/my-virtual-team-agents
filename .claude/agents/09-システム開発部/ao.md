...
### 2026-09-09
- **よくある失敗：外部サービス（Slack通知・LINE Notify等）からのWebhook受信エンドポイントに署名検証を実装せず、送信元IPやペイロード形式のゆるいチェックだけで受理していたため、URLを知る第三者が偽装リクエストを送ってDB操作を発火できる状態になっていた**。回避策は全Webhook受信エンドポイントで送信元が発行する署名（HMAC等）をヘッダから検証し、検証失敗は401で即拒否する実装をテンプレ化。新規Webhook追加時のPRレビューチェックリストに「署名検証の有無」を必須項目として追加
- **よくある失敗：一覧APIの`limit`パラメータにデフォルト値も上限値も設定せず、フロントが`limit`を渡し忘れると全件返却され、応募数が数万件に増えた案件で管理画面が固まる**。回避策はZodスキーマで`limit`に`.default(50).max(200)`のような既定値と上限を必ず設定し、上限を超える`limit`指定は422で拒否。ページング未指定APIの新規作成をlintで検出しリリース前に警告
- **よくある失敗：「今日」「今月」等の相対日付フィルタをサーバーのタイムゾーン（UTC）でそのまま計算し、JST 0:00〜8:59に応募したデータが「前日扱い」になって採用担当の日次集計が実態とズレる**。回避策は日付範囲計算を必ずユーザーTZ（JST）で算出してからUTCの半開区間に変換する共通関数に統一し、直参照の`new Date()`や`Date.now()`ベースの相対計算をlintで禁止する
- **よくある失敗：本番環境変数の追加をVercelダッシュボードで手動設定し、Preview環境への反映を忘れたため、PRのプレビューデプロイだけ機能が動かず「ローカルでは動くのにPRで壊れる」原因調査に時間を溶かす**。回避策は環境変数を追加した際に`.env.example`への追記だけでなく「対象環境（Production/Preview/Development）」を明記したチェックリストをKuuへの申し送りに含め、3環境すべてへの設定完了を実装完了レポートの必須項目にする
...
### 2026-08-27
- **[更新] 07-LP 部 ren/tsumugi へ着手前に渡すのは Zod スキーマと統一エラー DTO だけでなく「成功レスポンスの中身」までをセットにする（旧 2026-08-13 を更新）**：フィールド名の命名揺れ（`name` か `fullName` か）を潰しても、成功時に何が返るかを渡していないと ren の完了画面は「送信しました」だけになり、求職者に控えが残らず、後日の電話問い合わせで採用担当が本人を特定できない。成功レスポンスに「人が読める受付番号（内部 UUID とは別の連番）＋JST の受付日時」を含める契約を STEP 0 で確定し、ren には完了画面への表示、tsumugi には自動返信メールへ同じ値の記載を指示する。Server Actions 経由でも「フォーム由来だから」と認可を省かず `checkUserOwnership()` を必ず通す原則は従来どおり維持
...
### 2026-09-01
- **ローカルの seed は数十件でなく本番相当ボリューム（応募1万件・添付付き）を既定にする**：Query Logging を常時オンにしても（2026-08-18参照）、データが数十件では N+1 もインデックス欠落も体感差が出ず、結局ステージング以降で発覚する。`seed --scale=production` を用意して応募・選考履歴・面接調整まで本番相当件数を投入し、書いた直後に遅いクエリが遅く見える状態を作る。採用担当が毎朝叩く応募一覧の全件・全期間表示も、ローカルで同じ重さを再現できる
...
### 2026-09-02
- **[更新] 外部連携は「ジョブキューに載せる」だけで終えず、トランザクション境界の外へ出す（Outbox）まで設計する（旧 2026-08-12 を更新）**：`void somePromise()` の投げっぱなしを lint で禁止し、実行結果を DB に永続化（pending/succeeded/failed）＋DLQ＋バックオフ再試行を必須化する方針は維持する。そのうえで、キュー投入や外部API呼び出しを `prisma.$transaction()` の内側で行うと、①外部の応答を待つ間 DB 接続を握り続けてサーバレスの接続プールが枯渇する ②その後ロールバックしてもメールだけ送信済みになり「応募レコードは無いのに完了メールが届いた」が起きる。応募レコードと同一トランザクションで `outbox` テーブルへ行を挿入し、コミット後に別ワーカーが拾って送信する形にして、DB の整合と外部送信の発火を同じコミットに束ねる。
...
### 2026-09-13
- **求職者は電波の不安定な現場・移動中の電車内から応募するため、送信失敗＝入力した職歴が全消えになり再入力されない**：職務経歴の自由記述を10分かけて書いた直後に圏外で失敗すると、その人はほぼ戻ってこない。冪等キーと3状態レスポンス（2026-08-27参照）は送信済み判定の話であって、失敗時の入力保全は別問題として設計する。下書き保存エンドポイント（部分バリデーションのみ・必須項目なしで受ける）を API 契約に含め、下書きID と本送信の冪等キーの対応関係を Ao 側で規定して ren に渡す。復元をフロントの sessionStorage だけに任せると、ブラウザのタブ落ち・OSのメモリ解放で消える
