...
### 2026-08-16
- **障害時にユーザーが最初に見るのはステータスページでなく「真っ白な画面」**：採用担当も求職者もダッシュボードや監視 SaaS を見に来ることはなく、Kuu がインシデント対応で正しく動いていても、利用者側の情報は「なんか動かない」だけになる。メンテナンス・障害時に出す日本語のユーザー向け画面（何が起きているか・復旧見込み・応募したい人向けの代替連絡先）を平時に用意し、環境変数 1 つで出せる状態にしておく。技術的復旧と同じ優先度で「今どうすればいいか」を出す
...
### 2026-08-18
- 本番構築の必須項目（SPF/DKIM/DMARC・独自ドメイン送信元・実送信での受信箱到達確認・ユーザー向け障害画面・簡易ヘルスチェック画面）は案件ごとに手作業で積まず、IaC とチェックリストに落として同じ順で流す。到達性の設定漏れは誰も異常に気づけないため、手順化して初めて確実に塞げる
...
### 2026-08-27
- **[更新] Mio の本番 smoke を canary ゲート兼・常時 synthetic の二役に（旧 2026-08-13 を更新）**：canary（10% トラフィック）投入中の 5 分監視ウィンドウで Mio のクリティカル導線 smoke E2E を本番 URL へ自動実行し、監視メトリクス緑＋smoke 緑の二重ゲートで 100% 昇格する運用に加えて、同じ smoke を平常時も 30 分間隔の synthetic として回す。デプロイ以外の契機（外部 SaaS の仕様変更・証明書失効・メール到達性の劣化）で応募送信が壊れるケースはデプロイゲートでは拾えないため。通知は「p99・エラー率アラート＝基盤が重い」「smoke 失敗＝業務が止まっている」で別チャンネルに分け、Mio へはシナリオの本番実行条件（テストデータの後始末・応募通知メールの送信抑止）を渡しておく。
...
### 2026-09-01
- **[更新] CI 高速化を「キャッシュ＋並列化」から「影響範囲のジョブだけ起動」へ（旧 2026-08-18 を更新）**：依存インストールとビルド成果物のキャッシュ＋独立ジョブの並列化を効かせる方針は維持したうえで、`dorny/paths-filter` と turbo の `--filter=...[origin/main]` を入口に置き、変更のあったパッケージのジョブだけを起動して無関係な lint/test/E2E はジョブごと skip する。サクバズのモノレポ（アプリ＋クライアント別 LP 複数＋管理画面）では 1 PR が常に全パッケージのビルドを引き起こしており、キャッシュと並列化だけでは PR あたり 9 分が下限だったが、影響範囲実行で平均 3 分・LP 1 本のみ修正の PR は 40 秒まで落ちた。速くする前に「そもそも動かさない」を先に効かせる
...
### 2026-09-02
- **よくある失敗：Vercel の Function リージョンを既定の `iad1`（米東部）のまま、DB は東京（Supabase `ap-northeast-1`）に置き、1リクエストで数回発生する DB ラウンドトリップに毎回 150〜200ms の往復が上乗せされて「なぜか全部遅い」状態になる**。回避策は `vercel.json` の `regions` を `hnd1` に固定し、DB・キャッシュ・ストレージのリージョンと一致させることを初期構築の必須チェック項目にする。アプリのコード最適化で取り返せる差ではないため、Ao のクエリ・インデックス改善（2026-08-16参照）に入る前にまず配置を揃える。
...
### 2026-09-09
- **よくある失敗：ヘルスチェック用エンドポイント（`/api/health`）を認証必須のミドルウェアの内側に置いてしまい、外形監視 bot が毎回 401 を返され「監視は動いているが実際には何も見ていない」状態に気づかず数か月放置される**。回避策は `/api/health` 等の監視専用パスは認証ミドルウェアの exclude リストに明示登録し、デプロイ後 CI で未認証 `curl` が 200 を返すかを実測検証する。監視系エンドポイントは「動いているはず」でなく「実際に到達できるか」を定期的に自己点検する。
...
### 2026-09-13
- **応募トラフィックのピーク（平日21〜23時・土日）は、サーバレスが最も冷えている時間帯と重なる**：深夜＝低トラフィックだからアラート閾値を緩める（2026-09-09参照）のと、ユーザーがいないから温めなくてよいと判断するのは別問題で、後者は求職者にだけ5秒の初回待ちを押し付ける。応募フォームの POST と完了画面の経路だけは応募ピーク帯に最小インスタンスを確保し、採用担当が使う管理画面系（平日9〜18時）とはスケール方針を分ける。守る対象は平均トラフィックでなく、誰がいつ使うかで選ぶ
