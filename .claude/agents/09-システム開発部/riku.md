...
### 2026-08-05
- **よくある失敗：リスト描画の `key` に配列 index を使い、並び替え・挿入・削除で入力中フォームの値や選択・チェック状態が別の行に付き変わる／再マウントで状態が飛ぶ**。回避策は `key` は必ずレコードの安定した一意 ID を使い、index キーは並び替わらない静的リストに限定。動的リストで index キーを使う箇所を ESLint/レビューで検出し、「並べ替えたら別の行の入力が消える」原因不明バグを構造排除する。
...
### 2026-08-12
- **よくある失敗：数千件のリストを仮想化せず全件を DOM に描画し、ノード数万でスクロールがカクつき・メモリ肥大・INP 悪化を招く**。回避策は長いリスト/テーブルは `@tanstack/react-virtual` 等でウィンドウ仮想化し、可視範囲＋バッファのみ描画。無限スクロールは `useInfiniteQuery`＋仮想化を組み合わせ、`IntersectionObserver` の cleanup も必須。1 画面に載る DOM ノード数を実測し、想定本番件数（数百〜数千行）で崩れないかを実装段階で確認する。
- **よくある失敗：トーストや非同期の結果・エラーを `aria-live` 無しで DOM に差し込み、スクリーンリーダー利用者に一切通知されず「送信できたのか失敗したのか分からない」**。回避策は動的通知は `role="status"`/`aria-live="polite"`（成功・進捗）、エラーは `aria-live="assertive"` の live region 経由で出し、視覚のトーストと同じ内容を読み上げさせる。フォーム送信結果・バリデーションエラーの読み上げを a11y の必須チェック項目化し、`axe-core` だけでは拾えない「通知の伝達」を実機 VoiceOver で確認する。
- **よくある失敗：電話番号・郵便番号・ゼロ始まりの ID を `<input type="number">` で扱い、先頭ゼロが消える・指数表記になる・ホイール/スピナーで誤入力される**。回避策は数値「量」でない識別子系は `type="text" inputmode="numeric" pattern="[0-9]*"` で文字列として扱い、`type="number"` は本当に数量計算する項目に限定。全角数字の正規化・最大桁バリデーションも Zod で合わせ、実機のキーボード表示（数字キーパッド）まで確認する。
- **よくある失敗：Suspense/ローディング境界を粗く（ページ全体を 1 つの `<Suspense>`）置き、一部の重いデータ取得のせいで画面全体が真っ白ローディングになり段階表示できず体感が遅い**。回避策は Suspense 境界を「ヘッダー・一覧・サイドバー」等の UI ブロック単位に分割し、速い部分を即表示・重い部分だけストリーミングで後追い。各ブロックに骨組み（スケルトン）を用意し、1 箇所の遅延が画面全体をブロックしない構成にして体感 LCP を稼ぐ。

### 2026-08-13
- **Nao との連携：Server→Client 境界を越える props は Date/Map/関数を直列化できないため、Nao の DTO 設計で「FE へ渡す値はプレーン化・日時は ISO 文字列」を握る**。RSC ペイロードの直列化制約を設計段階で共有しないと、Riku が受け取った Date をそのまま渡して Hydration ミスマッチで詰まる。境界を越えるデータ形式を Nao の API 仕様に明記してもらう。
...
### 2026-08-16
- **ユーザー視点：業務システムの実利用者は建設現場の屋外・直射日光下でスマホを見るため、デザイン上きれいな薄グレー文字（コントラスト比 3:1 台）は「画面が真っ白で何も読めない」状態になる**。コントラスト 4.5:1 はスクリーンリーダー利用者への配慮という文脈で語られがちだが、建設業クライアントの業務システムでは「屋外で使う全ユーザーの実用要件」。設計レビューで淡色指定が来たら a11y でなく「現場で読めない」を根拠に差し戻し、実機を屋外の明るさで確認する。
- **ユーザー視点：現場責任者は手袋・軍手をしたままタップするため、44px 未満のタップターゲットや行間を詰めた一覧の行タップ、密集したアイコンボタンは誤タップが常態化する**。回避策は現場で使う画面のタップ領域を最低 44×44px で確保し、削除・確定など不可逆な操作は隣接配置しない。入力もキーボード打鍵を減らす方向（自由入力より選択式のボタン・プルダウン）へ寄せ、「手袋で操作できるか」を実装完了の自己判定条件に入れる。
...
### 2026-08-18
- 現場向けUIの必須要件（44pxタップターゲット・コントラスト4.5:1・送信中disabled＋楽観的UI・localStorage自動下書き・行動指示型エラー＋自動フォーカス）は画面ごとに実装せず、共通コンポーネントとフックに畳み込む。実装時間と実機事故が同時に消える
...
### 2026-08-27
- **[更新] Mio への引き渡し条件に「共通化した範囲の一覧」を追加（旧 2026-08-13 を更新）**：コンポーネント単体の回帰（モーダル開閉・4 状態切替・バリデーション表示）は Storybook の `play` つきストーリーで Riku が担保し、Mio の E2E は画面横断導線に絞ってもらう層分担を実装完了報告の引き渡し条件にする。これに加えて「共通コンポーネント・共通フックへ畳み込んだ横断要件」（送信中 disabled ＋楽観的 UI・localStorage 自動下書き・44px タップターゲット・行動指示型エラー＋自動フォーカス）の一覧を報告に添える。共通側に 1 本テストがあれば全画面分の回帰が賄えるため、Mio が画面数ぶん E2E を積むのを構造的に止められる。
...
### 2026-09-01
- **[更新] 3 レイアウトテンプレからの派生を scaffold コマンド化する（旧 2026-08-18 を更新）**：画面をゼロから組まず一覧／詳細／フォームのテンプレから派生させる方針を、`pnpm gen:page` 相当の社内 scaffold へ落とし、ルーティング・OpenAPI 由来の型接続・空/読込中/エラー/権限なしの 4 状態・Storybook ストーリー・テスト雛形までを一括生成する。コピー&貼り付けでの派生は「前の画面の残りコード」が混入し、生成物のどこを直したのかが後から追えない。1 画面の初期組み上げが 4 時間から 30 分になり、状態の作り忘れ（空状態だけ未実装）が構造的に起きなくなる
...
### 2026-09-02
- **よくある失敗：`onChange` で即座に検索やバリデーションを走らせ、日本語 IME の変換確定前の文字（「けんせつ」の途中状態）でリクエストが飛ぶ・エラーが点滅し、氏名や会社名の入力が実用にならない**。回避策は `compositionstart`/`compositionend` を見て変換中は確定処理を抑止し、インクリメンタル検索は確定後＋デバウンス（300ms 目安）で発火させる。英語圏由来のサンプル実装をそのまま使うと必ず踏むため、日本語入力を伴う全フィールドを実装時の確認対象に含める。
...
### 2026-09-09
- **よくある失敗：印刷用スタイル（`@media print`）を一切用意せず、採用担当がブラウザの「印刷」で求人票・応募者一覧を紙に出すと、ナビゲーション・ボタン・背景色がそのまま印刷されて用紙をまたいで表が分断される**。回避策は主要な一覧・詳細画面に `@media print` で「ナビ非表示・背景色除去・表の改ページ制御（`break-inside: avoid`）」を最低限適用し、建設業クライアントのように紙運用が併存する現場では印刷確認を実装完了のチェック項目に加える。PDF 出力機能がない画面ほど、ブラウザ印刷が実質の出力手段になっていることを想定する。
...
### 2026-09-13
- **ユーザー視点：現場責任者はスマホを片手で持ち親指だけで操作するため、画面上部やヘッダーに置いた「保存」「次へ」は物理的に届かず、持ち替えか端末の持ち直しが毎回発生する**。回避策はモバイル幅で主要 CTA を画面下部の sticky bar に固定し、iOS のホームバー・Android のジェスチャー領域と重ならないよう `env(safe-area-inset-bottom)` 分の余白を必ず確保する。加えてソフトキーボード表示時に下部固定要素が入力欄を覆わないよう `visualViewport` で退避させる。親指の可動域（画面下 1/3）に主要操作を置くのは好みでなく、片手前提の現場での操作可否を決める要件。
