...
### 2026-08-27
- **[更新] STEP 3 前半のプロトタイプは工程に「置く」だけでなく、Nao の設計順序と Riku の使い捨て範囲まで指定して発注する（旧 2026-08-18 を更新）**：工程に置いても、Nao が全画面の設計を終えてから着手すると前半に間に合わず、Riku が本実装の作法（認可・バリデーション・エラー整形）で作ると1画面に実装工数が丸ごと乗る。Kai が「クライアントが最も判断に迷う1画面（採用担当が毎朝触る応募一覧など）」を名指しして Nao にその画面だけ先行設計させ、Riku には「捨てる前提／認可・バリデーションは省略／自社の実データ投入だけ必須」と範囲を明示して発注する。捨てる部分と流用する部分を先に決めないと、プロトタイプがそのまま本番に居座る
...
### 2026-09-01
- **進捗確認の同期会議は廃し、DoD 達成報告を「完了タスク／残リスク／ブロッカー」の3行非同期テンプレで受ける**：Nao・Riku・Ao・Kuu・Mio を集める同期会議は、各自が自分に関係しない報告を聞く時間を人数分ぶん発生させる。非同期の3行報告に置き換え、同期で押さえるのは合流点（FE/BE の結合・契約テスト・データ移行リハーサル）だけに限定する。完了率でなく残リスクで見る運用（既存方針）とも報告フォーマットが揃い、Kai 側の集約も貼り合わせで済む
...
### 2026-09-02
- **[更新] 外部依存は社内の専用レーンに分けるだけでなく、クライアント側の作業一覧として先方へ渡す（旧 2026-08-05 を更新）**：外部起因タスクを専用レーンへ分離し「期限・督促担当・未達時の代替案（暫定素材で先行実装）」を持たせる運用は維持したうえで、レーンが社内の管理ツールにしか無いとクライアントは自分に何件の宿題があるか知らないまま着手する。キックオフで「クライアントTODO表（項目／担当者の実名／期限日／想定所要時間／遅れた場合に全体が何日ズレるか）」を渡し、Akari 経由で週次に残件だけリマインドする。素材・承認・アカウント発行・検収は先方の稼働であり、可視化しない限り自社の督促工数として毎週消える
...
### 2026-09-09
- **よくある失敗：複数クライアント案件の優先順位を「声が大きい／直近で連絡が来た」順で属人的に判断し、実は納期が近い別案件が後回しになって遅延が発覚するのが直前になる**。回避策はポートフォリオ横断の案件一覧に「納期／クリティカルパス残日数／クライアント影響度」を機械的にスコア化した優先順位列を持たせ、Kaiのリソース再配分判断はこの列を見て行う運用に固定する。声の大きさでなく数値で優先度を決める
...
### 2026-09-13
- **[更新] 検収は期限とチェックリストだけでなく「誰が・いつ・何分使うか」をキックオフでカレンダー確保するところまで決める（旧 2026-09-02 を更新）**：STEP 0 の契約時点で検収期限（納品後◯営業日）・期限経過時のみなし検収・機能単位の部分検収を定義し、納品時に受け入れ基準の Given-When-Then を転記した検収チェックリストを添える運用は維持する。そのうえで建設会社の検収担当は工事部長や総務の兼務で、机に向かえる時間が1日30分あるかどうかのため、期限とチェックリストだけ渡しても「見る時間がない」で滑り、請求が1〜2ヶ月止まる構図は変わらない。キックオフ時点で検収を1時間の同席セッションとして担当者のカレンダーへ先に入れ、当日は自社の実データでチェックリストを一緒に上から潰す。検収は先方の善意でも期限でもなく、確保された時間でしか動かない
