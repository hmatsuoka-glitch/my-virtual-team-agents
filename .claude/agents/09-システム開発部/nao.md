...
### 2026-08-16
- **ヒアリングで担当者が語る業務フローは「あるべき姿」で、実運用の 3 割は例外経路で回っている**：電話で来た応募を担当者が代理入力する、紹介・知人経由で選考が始まる、面接日程が LINE で決まって後から入力される、といった経路は最初のヒアリングでは出てこない。STEP 1 で「先週実際に来た応募を 10 件、どこから来たか全部教えてください」と実データで聞き、例外経路を後付けの裏技でなく必須ユースケースとして設計に組み込む。例外を持たない設計は現場で Excel 併用に戻される
...
### 2026-08-18
- STEP 1のヒアリングは業務フローを抽象的に聞かず、「先週実際に来た応募を10件、どこから来たか全部教えてください」の実データ質問を定型手順にする。例外経路が最初の1回で出揃い、設計後の要件追加が構造的に減る
...
### 2026-08-27
- **[更新] Kuu との通知台帳の突合に「宛先種別×気づき SLA」を追加（旧 2026-08-13 を更新）**：通知台帳（宛先・種別・状態・送信日時・失敗理由・再送回数）を Kuu のキュー・cron・再送基盤と設計段階で突合する枠組みに、業務要件として「通知先は担当者個人か共有アドレスか」「何分以内に気づく必要があるか」を先に確定して加える。この 2 つが決まると再送間隔・エスカレ先・失敗時の運用者通知が逆算でき、Kuu 側の設定値まで一気に埋まる。担当者個人宛だけの設計は外出・退職・共有漏れで簡単に破綻するため共有アドレス＋複数名同報を既定にし、Outbox（書き込みの原子性）と台帳（届いたかの追跡）の二層を Kuu と分担して設計する。
...
### 2026-09-01
- **[更新] 用語・ステータス・ID 体系の単一定義表を機械可読 YAML にして各層へ生成する（旧 2026-08-18 を更新）**：設計書に定義を書き写さず Ao の OpenAPI と Riku の画面仕様が単一の定義表から派生する方針を、表を `domain.yaml`（用語・日本語表示名・ステータス値と遷移・ID 採番規則・バリデーション制約）として機械可読に置き、そこから Zod スキーマ・OpenAPI の enum・DDL の CHECK 制約・画面のラベル定数を生成する形へ進める。人が 3 箇所に写す運用では「設計は `in_review` だが実装は `reviewing`」のズレが必ず残り、結合時に発見される。生成に切り替えてから設計変更 1 件あたりの反映作業が 4 箇所 30 分から 1 ファイル 3 分になり、層間の定義ズレ起因の差し戻しはゼロ
...
### 2026-09-02
- **よくある失敗：CSV/Excel の一括取込を「ファイルを読んで INSERT」としか設計せず、文字コード（Shift_JIS）・列順の入れ替わり・1 行の不正値で全件ロールバックするか・エラー行の返し方が未定義のまま実装へ渡り、現場で「取り込めないが理由が分からない」が常態化する**。回避策は STEP 2 で取込仕様を「受理する文字コードと改行コード・ヘッダ行の照合方法・部分成功か全件ロールバックか・エラー行の返却形式（行番号＋列名＋理由の CSV 返却）・再取込時の重複判定キー」の 5 項目で確定する。建設業クライアントは既存の Excel 台帳を持ち込むため、取込は付帯機能でなくデータ移行の成否を決める主機能として設計する。
...
### 2026-09-09
- **よくある失敗：段階リリース・A/B テストの要件が後から追加されることを想定せず設計し、実装完了後に「一部ユーザーだけ新機能を見せたい」となって、if 分岐がコンポーネント・API・DB 各層にアドホックに埋め込まれる**。回避策は STEP 2 で「将来的に段階公開が要る機能か」を機能一覧に 1 列追加し、該当する機能は最初から Feature Flag（環境変数でなく管理可能なフラグテーブル or LaunchDarkly 等）を通す設計にしておく。全機能に先回りで入れる必要はなく、フラグが要る/要らないを明示的に判定した記録を残すことが目的。
- **よくある失敗：外部サービス（決済・求人媒体）からの Webhook 受信を「URL を叩かれたら処理する」程度の設計で済ませ、署名検証・タイムスタンプ検証を書き落とし、第三者が同じペイロードを再送（リプレイ）すると同じ処理が何度も走ってしまう**。回避策は Webhook 受信の設計に「署名ヘッダーの検証方式」「許容タイムスタンプ差（例 5 分以内）」「処理済みイベント ID の重複排除（idempotency）」の 3 点を必須セクション化し、Ao が実装時に迷わない粒度で仕様化する。外部からの入力は全て「なりすまし・再送される前提」で設計する。
- **よくある失敗：多言語対応（i18n）を「日本語だけで作って後で対応すればいい」と後回しにし、実装が進んでから文字列を洗い出すと、JSX に直書きされた日本語が数百箇所に散らばっていて置換に数週間かかる**。回避策は初期段階で多言語化の可能性を Kai に確認し、可能性があるなら最初から `next-intl` 等の i18n ライブラリと翻訳キー方式を導入コストの低い段階（画面数が少ないうち）で仕込む。「今は日本語のみ」でも将来対応の芽があるなら、文字列を直書きしない規約だけは先に敷く。
- **よくある失敗：外部公開 API のバージョニング戦略を決めないまま `/api/xxx` を素朴に生やし、後から破壊的変更（フィールド削除・型変更）が必要になった時に既存クライアント（求人媒体・提携先）を壊さず変更する手段がなく、全連携先と個別調整する事態になる**。回避策は STEP 2 で「このエンドポイントは内部専用か外部公開か」を判定し、外部公開なら着手時点で `/v1/` プレフィックスとフィールド非破壊ルール（追加は自由・削除は非推奨化を経て `/v2` で実施）を設計書に固定する。内部 tRPC は都度変更可、外部契約 API は非破壊、と層で運用ルールを分ける。
