...
### 2026-08-27
- **Kuu との連携：バグ再現に使う PR preview 環境が自動 teardown で消える前に、起票済み不具合の PR へ「保持 72 時間」の例外タグを付ける運用を Kuu と決める**。Kuu 側は放置環境の GC を cron で回しているため、Mio が翌朝に再現手順を確認しようとすると環境ごと消えていて、再現待ちのために再デプロイを頼むことになる。起票と同時にタグを立て、期限で自動削除、を Mio の起票フローに組み込む。
...
### 2026-09-01
- **[更新] 実運用由来の境界ケース常設スイートを案件横断の共有パッケージへ（旧 2026-08-18 を更新）**：戻る後の再送信・多タブ同時編集・貼り付け由来の全角/不可視文字・送信連打・オートフィルを案件ごとに考え直さず常設スイートとして持ち回す方針を、`@let/qa-presets` 相当の社内パッケージ（Playwright fixture ＋シナリオ ＋ モバイル/throttling のプロジェクト定義）に固めて npm 参照する形へ進める。ファイルをコピーして回すと案件ごとに微修正が入って原本がどれか分からなくなるが、パッケージ参照なら改善が全案件へ一度に波及する。案件開始から常設スイートが緑になるまで 2 日かかっていた立ち上げが半日に、スイート改修の反映漏れもゼロに
...
### 2026-09-09
- **よくある失敗：Visual Regression のベースライン画像をローカル（macOS）で撮ったまま CI（Linux）で比較し、フォントレンダリングの微差（アンチエイリアシング）で毎回数 px の diff が出て「常に赤」になり、誰も画像 diff を見なくなる**。回避策はベースライン画像は必ず CI と同一の Docker イメージ・フォントセットで撮り直すルールを固定し、ローカルでの visual test 実行はスキップして CI 生成物のみを正とする。diff 許容領域は `maxDiffPixelRatio` で明示的に持たせ、環境差起因のノイズをアサーションの外に出す。
...
### 2026-09-13
- **ユーザー視点：起票した不具合の再現手順を内部用語（`job_posting` テーブル・seed ID・`/admin/applications`）で書くと、クライアント側の採用担当は自分の環境で再現できず「うちでは起きません」と平行線になる**。回避策は再現手順を画面に表示されている名称と実務語（「求人一覧」「応募者の詳細」「選考状況を『面接設定』に変更」）だけで書き、前提データも「担当者権限のアカウントで／応募が 1 件以上ある状態で」と業務条件で記述する。クライアントが手順を読んで 3 分以内に自力で再現できることを起票完了の条件にすると、Kai を経由した確認往復が 1 往復で済む。
