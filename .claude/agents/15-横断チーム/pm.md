...
### 2026-08-18
- 報告は正式記録（Notion/報告書）と到達手段（LINEへ要点3行＋リンク）を最初から分離した2種セットのテンプレで運用する。どちらかを後から作り直す作業が消え、記録の正本性を保ったまま到達率だけを上げられる
...
### 2026-08-27
- **Bo連携：自動化案件の検収は「動くこと」でなく、現場向け1枚（停止権限・停止手順・復旧手順／Bo 08-18記録）を毎日触る担当者が読んで停止操作を一度試したことをゲートにする**：検収チェックリストの実操作確認（08-16記録）の対象に、正常系の操作だけでなく停止操作を必ず含める。Boは停止手順を復旧手順より前に置く設計へ変えたが、検収で一度も試されないと異常時に現場はBoへの確認待ちで誤処理を眺めるだけになる。確認済みの担当者名はCSハンドオフ（08-13記録）へそのまま引き継ぐ。
...
### 2026-09-01
- **効率化テクニック：建設案件の進捗は現場に聞かず、Owlの標準遷移ログ（受注→着工→搬入→完了→請求確定）をWBSマイルストーンへ自動写像して拾う**。日次の聞き取りは現場が電話に出られる時間帯に縛られ、PM側の待ち時間がそのまま進捗更新の遅れになる。ただし取り込み時に「実時刻の自動打刻」と「事務所で後からまとめて押される手入力遷移」（08-27記録）をフラグで分け、後者は最大半日ずれる前提でクリティカルパス判定（07-01記録）の対象外にする。区別せず自動化すると入力遅れを工程遅延と誤読して不要なリプランを起こし、自動化が逆に工数を生む
...
### 2026-09-02
- （よくある失敗）外注・協力会社が入る工程を「着手日から作業できる」前提でWBSに置き、選定・見積取得・発注書発行・下請法の書面交付にかかる発注リードタイムを工程に計上しない → 回避策：外注タスクの先行に「選定→見積取得→発注書発行→書面交付」を明示タスク化し、リードタイムは過去実績（5営業日前後）で持つ。ピーク週競合で外注判断（08-05記録）へ切り替える場合もこの日数を差し引いて間に合う週かを判定する（理由：キャパシティ・プランニング・08-03記録でピーク週を先読みしても、外注の立ち上げ日数を0日と置くと「外注すれば回る」判断が実際には間に合わず、結局残業前提の挽回・07-01記録に戻る。HRへの調達依頼・08-13記録の起点日もこの逆算で決まる）
...
### 2026-09-09
- **失敗パターン: 予算（外注費・工数）の消化率を納期直前まで確認せず、進捗は順調でも予算だけが先に枯渇し、残タスクを消化する原資がない状態で発覚する** → 回避策: WBSの予算管理に「進捗率に対する予算消化率」の対比を週次で自動算出し、消化率が進捗率を一定以上（例: 20pt）上回ったらWARNINGを出す。予備費10%（05-22記録）の消費ペースも同じ表で監視する。
...
### 2026-09-13
- **[更新] クライアント窓口（事務員）視点：依頼は「窓口の一存で返せるもの」と「社内決裁が要るもの」を最初から別便に分け、決裁要の依頼だけ相手社内の回付日数から期限を逆算する（旧 2026-08-16 を更新）**：旧運用は依頼に社内の承認者名と転送用文面を添えるところまでで、窓口の伝達コストは下がったが、決裁が要る依頼と要らない依頼が同じ1通に混ざるため「重い方が片付くまで全部止まる」構造が残っていた。建設クライアントでは社判・稟議の回付が週1の役員会や現場巡回の合間に乗るため実質3〜5営業日かかり、素材提出のような即答できる依頼までそこに巻き込まれる。別便にすれば軽い依頼は当日〜翌日で返り、決裁要の依頼だけ回付日数を織り込んだ期限を設定できる。社内承認者名と転送文面の添付（クライアントToDo／06-23記録）は決裁要の便にのみ維持する
