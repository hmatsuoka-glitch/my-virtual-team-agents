...
### 2026-08-18
- 分析の出力テンプレを「着任見込み人数と時期を先頭・CVR等は内訳として下層」の構造で固定する。指標のまま出して再質問を受ける往復が消え、経営向けと現場向けの出し分けも同じテンプレの階層で処理できる
...
### 2026-08-27
- **[更新] CS連携：解約理由の定性は自由記述で受けず、「値段／成果／担当相性」の分類コードをCSと事前定義して入力段階でコード化してもらう（旧 2026-08-13 を更新）**：チャーン分析のリスク顧客をCSへ渡し、定性理由を生存時間分析（08-03記録）の共変量に組み込む方針は維持する。ただし自由記述のまま受け取ると共変量化のために後から遡ってコーディングすることになり、解約から時間が経った案件ほど記憶バイアスが乗る。CSのヘルススコア入力フローに解約時の3分類（＋自由記述は補足欄）を必須項目として置いてもらい、Datは分類コードを共変量、自由記述を解釈の材料として層を分けて扱う。
- **Kpi連携：対外報告に添える業界・同規模の参照値（Kpi 08-16/08-18記録）はKpiがマスタから引き、算出定義の互換性判定はDatが担う役割分担にする**：Datが返すのは参照値そのものでなく「母数・調査年・地域粒度・自社指標との定義差」の4点セットで、建設採用のように地域・職種で水準が割れる指標は少母数なら参考値ラベル（08-05記録）を必須にする。互換性未確認の参照値はKpiがダッシュボード・対外報告に載せない運用と対で機能させ、Datが値そのものを配る側に回らない。
- **Pm連携：リスク登録簿の「発動トリガー」（Pm 08-12記録）も、クローズ条件（07-16記録）と同じくDatが定量条件を設計して返す**：Pmは各リスクにトリガー・発動時アクション・コスト負担者・判断者を事前定義する運用へ変えたため、「消えたと言える条件」だけ渡すのは片手落ちになる。発動側は誤警報を避けるため単発の閾値超えでなく「◯週連続で◯を超えたら」の持続条件で設計し、端点効果（08-12記録）で未確定の直近期がトリガーを引かないよう「確定済み期間で判定」を条件文に明記して渡す。
- **Marketing連携：採用支援の「着任見込み人数と時期」（08-16記録）を求められたら、面接以降の歩留まりデータを取得できるかを着手前に確認してから受ける**：応募までは媒体・GA4で取れるが、面接→内定→入社はクライアント側の管理にしか存在しない。Marketing経由で歩留まりの実数が取れない案件は「媒体CVまでで打ち止め・着任見込みは出せない」と着手前に明示し、代わりに過去案件の歩留まり中央値による試算へ切り替える旨を先に合意する。取れない前提を後から出すと、経営報告の直前に主指標が空く。

### 2026-09-01
- **効率化テクニック：営業日正規化・カレンダー差補正・確定期/未確定期の区別（08-12記録）を各分析に書かず、期間ユーティリティ1本に閉じ込めて全クエリから呼ぶ**。お盆・GW・週休二日で営業日数が割れる建設クライアントの前年同月比は、案件ごとに補正を書くと補正忘れと補正方法のばらつきが同時に起きる。「暦月→営業日あたり正規化」「最新期を未確定としてトレンド判定から除外」を関数化してCTEの前処理（06-16記録）に常駐させると、7社分の月次で毎回発生していた補正作業が消え、端点効果での誤警報も実装側で潰れる
...
### 2026-09-02
- （よくある失敗）A/Bテストや前後比較の結果を毎日覗き、有意が出た時点で打ち切るため（peeking／逐次停止）、名目5%の偽陽性率が実質20%以上に膨らむ → 回避策：開始前に必要サンプルサイズとMDE（最小検出可能効果）を算出して終了条件を先に固定し、途中で見るならα消費関数を用いた逐次検定に切り替える（理由：建設採用の少母数・08-05記録では必要期間が数ヶ月に及ぶことも多く、期間を先に合意しないと「早く出た良い結果」だけが採用され、施策判断が系統的に楽観側へ歪む）
- （よくある失敗）セグメント別ではすべて改善しているのに全体は悪化するシンプソンのパラドックスを、構成比の変化を確認せず「施策が効かなかった」と報告する → 回避策：全体の増減は必ず「各セグメントの水準変化」と「セグメント構成比の変化」へ要因分解してから結論を書く（理由：加重平均・08-12記録で集計方法が正しくても、低CVRチャネルの構成比が増えただけで全体CVRは下がるため、集計が正しいことと解釈が正しいことは別問題になる）
...
### 2026-09-09
- **失敗パターン: クライアント満足度・従業員アンケートの回答率が低い（例えば30%未満）まま「全体傾向」として報告し、回答した層（不満が強い/協力的な層）に偏った結果を母集団全体の意見であるかのように扱う** → 回避策: 回答率を必ず明記し、回答率が閾値（例: 50%）未満の調査は「回答者傾向」であり母集団代表値でないと限定表現に切り替える。非回答者との属性差（部署・在籍年数等）を可能な範囲で確認し、偏りが疑われる場合はlimitationsに明記する。
...
### 2026-09-13
- **クライアント経営者視点：レポートを信じるかは分析の正しさでなく「自分の知っている1件が正しく載っているか」で決まる**：社長は必ず自分が面接した応募者や知っている現場の数字を最初に探し、照合できないと全体集計そのものを疑う。主指標である着任見込み人数と時期（08-16記録）の直下に、直近の応募者リスト（応募日・流入経路・現在ステータス）を数行だけ添え、既知の1件が合っている事実で全体の信頼を担保する。個票を添えると突合可能性が上がる一方、氏名等は下4桁・イニシャル等に丸める線引きをKpi・Legalと事前に握る。
