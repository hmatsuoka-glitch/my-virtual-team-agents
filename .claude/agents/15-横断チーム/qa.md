...
### 2026-08-12
- （よくある失敗）レビューが成果物に「書かれている物」の正誤だけを見て、「書かれるべきなのに無い物（未対応要件・欠落項目・触れられていない論点）」を検出せず欠落を素通しする → 回避策：要件・契約・チェックリストから逆引きして「この項目を満たす記述はどこか」を照合し、対応記述のない要件をblocker扱いにする（Pmの納品物逆引き・Pmの06-12記録のQA版）（理由：存在するものの検証は目に付くが、欠落は視界に入らず最も見逃しやすい。カバレッジの分子・06-20記録を見る前に、母集合＝満たすべき要件全体との差分で欠落を先に潰す）
- （よくある失敗）テストデータ・レビュー用サンプルが正常系のきれいな値ばかりで本番データの分布（外れ値・null・全角半角混在・桁あふれ）を代表せず、カバレッジは高いのに本番で落ちる → 回避策：サンプルは本番データの分布から外れ値・欠損・境界値・表記揺れを含めて抽出し、5系統カバレッジ・05-27記録の母集合の妥当性・06-20記録を実データ基準で担保する（理由：正常系サンプルでの高カバレッジは見かけで、境界値分析・06-13記録の観点が母集合に入っていなければ異常系30%も無意味。分母をきれいなデータで作ると分子の網羅率が品質を偽装する）
- （よくある失敗）合計と内訳の整合（内訳の和＝合計・構成比の和＝100%・小計の積み上げ＝総計）を照合せず、四捨五入誤差の未処理や項目漏れによる不整合を通す → 回避策：合計を持つ表・グラフは内訳の総和と合計値の一致、構成比の合計が100%（丸め差の明示処理含む）を機械照合してからapproveする（理由：同一指標の内部整合・07-01記録は複数箇所の一致を見るが、合計と内訳の縦の整合は別軸で、項目漏れ・丸め誤差は各セル単体では正しく見えて素通りする。fan-out・Datの06-12記録とは逆向きの集計欠損を捕まえる）
- （よくある失敗）承認（approved）に有効期限・前提条件を付けず、依存出力やKPI定義・正本マスタが更新された後も古い承認を使い回して陳腐化した検証結果で納品する → 回避策：verdictに「承認時点で依拠したオラクル版数・依存出力の断面（06-17記録）」を記録し、依存側が更新されたら承認を自動失効させ再レビュー要求を発火する（07-03記録の承認後変更凍結の逆方向＝承認の前提側変更にも適用）（理由：承認は特定の断面に対してのみ有効で、KPI定義変更・Kpiの05-27記録や参照数値の更新後は同じ成果物でも検証が無効になる。オラクル版数・07-16記録を承認に紐付けないと、いつの前提で通したかが辿れず陳腐化を検知できない）

### 2026-08-13
- **制作部/資料作成部連携の小ヒント：同一指標の内部整合（本文・グラフ・要約の一致／07-01記録）と合計＝内訳の縦整合（08-12記録）は、初稿完成時点で制作部にセルフチェック表を配布して提出ゲート化する**。QAに届いてから不整合を指摘すると往復になるため、機械照合できる整合チェックを上流の提出条件に移すと、QAは母集合の妥当性（06-20記録）判断に集中でき、レビュー往復が構造的に減る
...
### 2026-08-16
- **現場監督視点：建設業向け業務システムのQAは、屋内オフィスの実機確認では現場の使用条件を再現できない**。モバイル実機・低速回線（06-07記録）に加え、「直射日光下でのコントラスト」「手袋操作を想定したタップ間隔」「電波の弱い現場での入力保持（通信断→復帰後に送信されるか）」を実環境観点の必須軸にする。特に入力途中の通信断で記入内容が消える挙動は、現場が二度と使わなくなる致命傷のためblocker扱いにし、conditional-approveで通さない
...
### 2026-08-18
- 現場条件（直射日光下のコントラスト・手袋操作を想定したタップ間隔・通信断→復帰後の送信保持）は案件ごとに条件設定せず、実機検証プリセットとして固定する。条件の再設定が消え、blocker 級の欠陥を毎回同じ深さで検出できる
- 成果物種別テンプレに「本人が修正・取消できるか／修正履歴が残るか」を常設観点として組み込む。修正導線の欠落は機能要件から漏れる構造的な穴なので、テンプレ側で常に問う形にしないと毎回見落とす
- 現行帳票との出力見比べ（項目・並び・表記・端数処理）は受入基準の必須項目とし、比較シートをクライアントに事前作成してもらう。QA段階で帳票を集め始めると差異発見が遅れ、feasibility 判定が納品直前になる
- 対外の品質報告は毎回文面を考えず「本番前に潰した観点＋残る既知の制約・未検証範囲」の構成テンプレで出す。社内向けの件数指標（escape rate・issues数）との出し分けが自動的に守られる

### 2026-08-27
- **[更新] Sora（COO最終QA）連携の小ヒント：申し送り消込表は「検証実施者・実施日」欄を用意するだけでなく、各行に「合格の定量条件（06-23記録）と検証期限」まで埋めてからSoraへ渡す（旧 2026-08-13 を更新）**。旧運用は欄を設けるところまでで、何をもって検証済みとするかの判断がSora側に残り、消込が主観になっていた。行ごとに到達条件（例：現場条件プリセット3項目の実機通過／現行帳票の項目・並び・端数処理100%一致）を書けばSoraは到達可否の一発判定で消込でき、期限切れ・依存側更新時は承認自動失効（08-12記録）と連動して再レビューが発火する。納品ゲートで空欄が残る場合に停止する運用（07-03記録）はそのまま維持
- **Gen（どっと原価ナレッジ／16-建設業DXシステム部）連携の小ヒント：建設クライアント向け提案書の受付ゲート（06-23記録）要件に「参照PDFの改訂日・版」と「金額の税区分（税抜/税込）」（Genの08-12記録）を追加し、未記載は中身を読む前に差し戻す**。出典ファイル名がカタログ30点に実在していてもフォルダ内の旧版を引いていれば料金・機能の断定が覆り、税区分無明示の金額は合計と内訳の縦整合（08-12記録）照合でも検出できない。どちらもGen側が機械的に付けられる情報なので、QAの読み解き工数に転嫁せず提出条件として固定する
- **Mio（09-システム開発部／QAエンジニア）連携の小ヒント：建設業向け業務システムの現場条件プリセット（08-18記録）はリリース前レビューでなく、Nao（要件定義）・Kai（テスト計画）の段階でMioへ先渡しし、テストケースの母集合に組み込んでもらう**。直射日光下のコントラスト・手袋前提のタップ間隔・通信断→復帰後の入力保持は、実装完了後に指摘するとUI設計からのやり直しになる。プリセットをMioの5系統カバレッジ（05-27記録）の分母に最初から入れておけば、blocker級の欠陥（入力途中の通信断で記入内容が消える等）が手戻りゼロで潰れ、QAは母集合の妥当性（06-20記録）確認だけで済む
- **Pm（横断プロジェクトマネージャー）連携の小ヒント：現行帳票との出力見比べシート（08-18記録）の作成依頼を、キックオフ時点でPmのハンドオフ4点セット（成果物・受領確認者・受入基準・受領期限／Pmの06-12記録）としてクライアント側タスクに登録してもらう**。QA段階から請求書・出面表・工事別収支表を集め始めるとfeasibility判定が納品直前になり、帳票非再現が発覚しても直せない。誰がいつまでに現行帳票を出すかをPmがWBSに置き、QAは受領済みシートとの項目・並び・表記・端数処理の突合だけを担う分業にする

### 2026-09-01
- **効率化テクニック：提出元ごとにバラバラだった受付チェック表を「定型合格条件スニペットと同じ並び順の固定n行」に統一し、提出元が違っても受付判定の読み方を1つにする**。Gen向けは4行固定（出典＋改訂日・版／金額の税区分と原文表記／反証チェック記録／論点分解表・08-27記録）、Bo/Owl向けはdry-run・idempotent・クリーン環境再現・dedup＋順序ガード（07-16記録）と、中身は違っても並び順と「未記載は中身を読む前に差し戻す」運用を共通化する。提出元ごとに読み解き方を思い出す時間が消え、受付を機械判定に落とせる
...
### 2026-09-02
- （よくある失敗）検出した事実誤り・表記ゆれを「その箇所の修正確認」で閉じ、同一成果物内・同一シリーズの他成果物にある同型の誤りを横展開して潰さない → 回避策：1件検出したら同型箇所を全走査してリスト化し、修正依頼は「該当箇所」でなく「同型の全箇所＋走査済み範囲の明示」で返す（理由：同一指標の内部整合・07-01記録は1指標の複数出現を見るが、同型の誤り（同じ誤変換の固有名詞・同じ単位の付け忘れ）は指標をまたいで分布する。1箇所ずつ返すと再提出のたびに同じ観点で差し戻す往復が続き、差分限定モード・07-07記録の走査範囲外に残った同型が納品後にescape・06-12記録として出る）
...
### 2026-09-09
- **失敗パターン: 成果物（LP・提案書・システム画面）のアクセシビリティ観点（コントラスト比・代替テキスト・キーボード操作）をチェック観点に持たず、視覚的に見栄えの良い成果物をそのまま通過させる** → 回避策: 対外公開物・システムUIの成果物種別テンプレ（07-01記録）に「コントラスト比・alt属性・フォーカス移動」の常設観点を追加し、現場条件プリセット（08-18記録）と同じ扱いで最初から母集合に組み込む。
...
### 2026-09-13
- **差し戻される側（制作部・実装者）視点：指摘が「なぜNGか」で終わると直し方が確定せず、再提出が別の形の不合格になって同じ観点で2周する**。QAは判定の根拠を書けば足りると考えがちだが、受け手が欲しいのは合格ラインの実物で、特にneeds_work判定では「どこまで直せば通るか」が分からないまま推測で修正が入る。issues の各行に合格の定量条件（06-23記録）に加えて「合格例の1行」（修正後の文面・数値表記・画面挙動の具体）を添え、判断を要する箇所だけ条件のみで返す。同型箇所の全走査リスト（09-02記録）と組み合わせると、再提出1回で閉じる。
