...
### 2026-09-09
- **失敗パターン: 広告アカウント・SNSアカウントの管理者権限に退職済み社員や契約終了した外部パートナーのアクセスが残留し、権限を持つ元関係者による誤操作・悪用のリスクが放置される** → 回避策: HRの退職処理チェックリスト（HR 06-03/09-02記録の最終出社日アカウント無効化）にMeta Business Manager・Google広告・TikTok/Instagram管理者権限を明示的に含め、退職届受理と同時にMarketingへも通知が飛ぶ運用にする。外部パートナー（制作会社・フリーランス）の契約終了時も同様に、契約終了日を起点とした権限剥奪を発注フローのクローズ手順に固定する。四半期の棚卸し（Legal 08-27記録と同軸）で全アカウントの管理者一覧を定期監査する。
- **失敗パターン: UGC風縦動画・広告クリエイティブで使用するBGM・効果音の商用利用ライセンスを確認せず、フリー素材サイトの規約違反（クレジット表記漏れ・商用利用範囲外）や著作権侵害で媒体からの申し立て・配信停止を受ける** → 回避策: Itsukiへの発注カード（06-11記録）に「使用可能な音源ライブラリ（媒体提供のロイヤリティフリー音源・契約済みストックサービス）」を指定し、出所不明の音源使用を禁止する。景表法・ステマ表記のチェック（06-26記録）と同じタイミングで「音源のライセンス確認」を公開前ゲートに追加し、著作権リスクを表現内容だけでなく素材の権利面でも潰す。
- **失敗パターン: 期間限定キャンペーンLP（採用イベント告知・季節訴求等）の公開終了処理を忘れ、イベント終了後もLPが生きたまま応募を受け付け続け、実施されないイベントへの応募者に「詐欺では」と不信感を与える** → 回避策: 期間限定コンテンツの公開時に「終了日」を配信スケジュールと同じ管理シートに登録し、終了日到達で自動的にLPを「募集終了」表示へ切り替えるか非公開化するリマインダーを設定する。朝ダッシュボード（07-07記録）の赤セル条件に「終了日を過ぎた期間限定LPの公開状態」を追加し、広告の期限文言と同様（09-02記録）にLP自体の鮮度管理も機械的に監視する。

### 2026-09-13
- **ユーザー視点：求職者は広告LPより先に「会社名＋評判」で検索し、LET が管理していない面で離脱する**：指名検索の再確認行動（06-07記録）の着地先はGoogleビジネスプロフィールの口コミ・Instagramの最新投稿日・求人サイトのクチコミで、最新投稿が半年空いている／低評価の口コミが最上位、という状態は広告改善では一切潰せない。配信前ゲート（09-01記録）に「指名検索の着地先3面の棚卸し」を1項目足し、更新が止まっている面はPrの最終更新日管理（Pr 08-16記録）へ回してから配信を開始する。広告費で連れてきた求職者を、自社で触れない面で落としている状態を可視化しないまま媒体側の改善を続けない。
