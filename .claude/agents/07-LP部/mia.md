...
### 2026-08-27
- **[更新] QA 用属性（`data-testid`／`data-qa-mask`）を Ren の共通コンポーネントパッケージ側に内蔵させ、しきい値とベースラインを部品単位で共有する（旧 2026-08-13 を更新）**：案件ごとに属性の付け忘れを Saki 経由で差し戻すルールでは、差し戻しの回数だけ QA 着手が遅れる。Ren がフォーム・固定 CTA・完了画面をパッケージ化した（ren 2026-08-18参照）のに合わせ、これら共通部品は属性を実装済みで出荷してもらい、Mia の領域別しきい値（Hero/CTA/Form 厳格・装飾は知覚判定）とセクション単位ベースラインもその部品に紐づけて共有する。属性チェックの対象が案件固有部品だけに縮み、共通部品では属性欠落による差し戻しが構造的に起きなくなる
...
### 2026-09-01
- **[更新] 親指到達域も `boundingBox()` の座標判定で機械化でき、人的 QA に残すのは「実機の体感速度」と「並列比較時の識別性」の2項目だけになる（旧 2026-08-18 を更新）**：親指到達域を目視オーバーレイで見る運用（2026-08-16参照）にしていたが、到達域は SP 幅ごとに画面下端からの Y 座標レンジで定義できるため、主 CTA と送信ボタンの `boundingBox()` を取って範囲内かを判定すれば CI 側で落とせる。コントラスト比・タップターゲット寸法・alt・見出し階層・1px ボーダー消失に到達域判定を加えて CI へ寄せ切り、人的 QA は機材条件を固定しても数値化できない体感速度と、案 A/B 並置時の識別性の2項目に絞る
...
### 2026-09-02
- **[更新] フォント検証は Network 配信の確認に加え、社名・職種名の実文字列でサブセット欠落まで照合する（旧 2026-08-05 を更新）**：検証マシンにローカルインストール済みのフォントで綺麗に見えているだけの偽合格を防ぐため、Network タブで webfont が実際に配信されているか（`local()` フォールバック依存でないか）を確認する運用は維持する。そのうえで日本語フォントはサブセット化されていることが多く、「髙」「﨑」「濵」等の外字を含む建設会社の正式社名・代表者名だけが別フォントや豆腐（□）で描画される欠落が、Lorem や短いダミー社名では一切表面化しない。STEP 3 の検査に「正式社名・代表者名・職種名の実文字列を流した状態でのレンダリング比較」を加え、欠落文字を検出したらサブセット範囲の拡張か当該箇所のフォールバック指定を Ren へ差し戻す。確認は等倍/2x だけでなく非整数 DPR（2026-08-16参照）でも同じ文字列で行う
...
### 2026-09-09
- **失敗パターン: フォーム送信エラーのトースト・スナックバー通知が fixed ヘッダーの z-index 配下に隠れて表示され、視覚 QA では「エラー表示あり」と機械的に検出されるがユーザーには実際見えていない** → 回避策: エラー通知系コンポーネントは `getBoundingClientRect()` でヘッダー等の他要素と重なっていないか座標判定を追加し、スクリーンショット比較だけでなく「見えている面積が閾値以上か」までQA項目に含める
...
### 2026-09-13
- **承認者は差分率を見ておらず、自分が知っている実物（正式社名の表記・ロゴの版・写真に写る現場）の正しさだけで合否を決める**：忠実度スコアが満点でも「髙」が「高」になっている、旧ロゴのまま、複製元の他社現場写真が残っている、のいずれか1つで納品全体が差し戻される。外字のサブセット欠落検査（2026-09-02参照）を「描画できているか」から一歩進め、正式社名・代表者名・許可番号を期待値文字列としてスコア表の自動生成JSON（2026-09-01参照）へ持ち、Kaitoの画像資産台帳（kaito 2026-09-02参照）の「複製元由来」区分が0件であることをQA通過条件に含める
