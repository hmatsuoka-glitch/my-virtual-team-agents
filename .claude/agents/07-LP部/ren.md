...
### 2026-07-11
- **業界用語再確認「Critical Rendering Path（クリティカルレンダリングパス）」の各段階と実装上の詰まり所**：HTML→DOM構築→CSSOM構築→Render Tree→Layout→Paint の初回描画までの経路で、CSS はデフォルトで render-blocking・同期 `<script>` は parser-blocking になる。LP の初期表示を速くするには「Above-the-Fold の CSS をインライン化・残りを `media` 分割で非ブロック化、JS は `defer`/`type=module`」が実装原則。`next/script` の `strategy` 選択もこの経路のどこをブロックするかで判断し、用語ベースで描画詰まりを切り分ける
...
### 2026-07-16
- **Mia の QA が安定するよう、実装側から `data-testid`（Hero/CTA/Form）と `data-qa-mask`（可変要素）を先に仕込んで渡す連携**：Mia はセレクタで領域別しきい値を当て、日付・カウンター・カルーセル等を `mask` で差分計算から除外するが、クラス名ベースで指定させると実装変更のたびに Mia 側の設定が壊れて偽差分が出る。骨格生成の STEP 1 時点で「厳格判定領域＝`data-testid`／除外領域＝`data-qa-mask`」を属性として付与し、Nao の計測イベント設計表の `data-testid` 列とも同じ値に揃える。QA 設定と実装の結合を属性に固定し、リファクタで QA が壊れる往復を消す
...
### 2026-08-12
- **失敗パターン: モーダル/ドロワー表示中に `body { overflow: hidden }` でスクロールロックし、閉じる時・アンマウント時に解除漏れでページ全体がスクロール不能に固まる** → 回避策: スクロールロックは必ず `useEffect` の cleanup（return）で解除し、複数モーダル同時表示に備えてロックをカウント管理（最後の1つを閉じた時だけ解除）。iOS Safari は `overflow:hidden` が効かないため `position:fixed`＋スクロール位置の保存/復元で実装し、閉じた後に元の位置へ戻す
- **失敗パターン: `next/image` の `fill` を `sizes` 無指定で使い、ブラウザが常に最大解像度画像を選んでLCP悪化・転送量増になる** → 回避策: `fill` 使用時は `sizes`（例 `(max-width: 768px) 100vw, 50vw`）を必ず指定し、SPで実表示幅に見合う解像度が配信されるかを DevTools Network で確認する。Hero画像は `priority` 付与（2026-05-15参照）とセットで、bundlesize/LCPの実測（Kaito 2026-08-03のAVIF確認と対）まで通す
- **失敗パターン: クライアントコンポーネントで `window`/`localStorage`/`Date.now()`/`Math.random()` を描画時（レンダー中）に参照し、SSRとクライアントの出力差でHydration mismatch・ビルド時ReferenceErrorを起こす** → 回避策: ブラウザ専用APIは `useEffect` 内かつ `typeof window !== 'undefined'` ガードで参照し、SSRで確定しない値（日時・乱数・localStorage依存の表示）は初期値をサーバー/クライアント共通（null等）にしてマウント後に反映する（2026-04-29のHydrationエラー回避の具体化）
- **失敗パターン: `.map()` の要素 key に配列 index を使う（または key を付け忘れる）ことで、並び替え・フィルタ・件数変動時に state や入力値が別の行へ引き継がれる/入れ替わる** → 回避策: key はデータ側の安定した一意ID（`item.id` 等）を使い、index キーは並びが不変の静的リストに限定する。フォーム項目・アコーディオン・動的リストで特に注意し、Reactの差分検出が要素同一性を誤認して入力が飛ぶ事故を防ぐ
...
### 2026-08-16
- **求職者は現場の休憩中に片手・親指でスクロールする前提で、固定CTAの位置と可視領域を実装する**：`100vh` 指定の固定CTAは iOS Safari のアドレスバー伸縮で下端が隠れ、ホームインジケータにも被って「押せないボタン」になる。`100dvh` ＋ `padding-bottom: env(safe-area-inset-bottom)` を固定要素の標準セットにし、CTAは親指の届く画面下 1/3 に収める。PC の DevTools では絶対に再現しない、実機だけで起きる「CTA が押せない」を実装層で潰す
- **スマホ応募フォームは `inputmode`／`autocomplete` の欠落だけで離脱する**：電話番号欄に `inputmode="tel"`＋`autocomplete="tel"` が無いとフルキーボードが出て、作業着・手袋の指では入力を諦める。氏名 `autocomplete="name"`、メール `inputmode="email"`＋`autocomplete="email"` を必須属性としてフォームテンプレに組み込み、求職者が「打てる」状態を既定にする。バリデーション強化より先に入力手段を整える
...
### 2026-08-27
- **[更新] 冪等キー＋pending disabled は Kaito と案件ごとに握らず、共通フォームコンポーネントへ内蔵して既定動作にする（旧 2026-08-13 を更新）**：Kaito の段階昇格・即時切戻し運用では旧バージョンを開いたままのユーザーが新 Server Action を叩く Version Skew が起きるが、案件ごとに合意する運用だと握り漏れた案件だけ無防備になる。クライアント生成 UUID の冪等キー・pending 中の disabled・送信失敗時の再送導線をパッケージ（2026-08-18参照）の既定に組み込み、Kaito 側は「フォーム有 LP＝Skew Protection 有効化」をテンプレート Vercel プロジェクト（kaito 2026-08-18参照）の初期設定に入れてもらう。合意事項を初期構成へ前倒しし、握り忘れの余地を消す
...
### 2026-09-01
- **[更新] 共通コンポーネントは案件リポジトリへコピーせず、社内パッケージとしてバージョン付きで配布し、修正が全案件へ1回で伝播する状態にする（旧 2026-08-18 を更新）**：パッケージ化しても実体をコピーして配ると、`inputmode` の追加や `100dvh`＋`env(safe-area-inset-bottom)` の修正が案件本数分の手直しになり、古い案件だけ実機事故が残り続ける。フォーム・固定 CTA・完了画面・Hero 3型骨格（2026-08-27参照）を GitHub Packages でバージョン固定配布し、案件側は依存バージョンを上げるだけにする。Mia の QA 属性としきい値も部品バージョンへ紐づくため、共通部品の検証結果を案件間で使い回せる
...
### 2026-09-02
- **[更新] 開閉 UI は ARIA の自前実装でなくネイティブ `<dialog>`／`<details>` を第一選択にする（旧 2026-08-05 を更新）**：`div`＋onClick の見た目だけのボタン禁止は前提として、role と `aria-expanded`/`aria-controls` を自前で当てる方針だと、フォーカストラップ・Escape での閉じ・背景要素の `inert` 化を案件ごとに手書きすることになり、実装差と抜けが必ず出る。モーダルは `dialog.showModal()`（トラップ・Escape・`::backdrop` をブラウザが担保）、アコーディオンは `<details>/<summary>` で組み、ARIA の手当ては両者で表現できないタブとメガメニューだけに限定する。ただし `showModal()` でも背景スクロールは止まらないため、ロックのカウント管理と iOS の `position:fixed` 退避（2026-08-12参照）は引き続き併用する
...
### 2026-09-09
- **失敗パターン: Service Worker 実装時にキャッシュ名をバージョニングせず固定文字列のまま運用し、修正デプロイ後もブラウザが古いキャッシュ済みアセットを配信し続けて「直したのに反映されない」となる** → 回避策: キャッシュ名にビルドID・コミットハッシュを含めて毎回変更し、`activate` イベントで旧キャッシュを `caches.delete()` する実装を SW 導入の標準テンプレに固定する
...
### 2026-09-13
- **求職者はフォーム入力の途中で電話・LINE・作業に中断され、戻った時にはタブが破棄されている**：iOS はメモリ逼迫時に背景タブを復元ロードするため入力途中の state は消え、氏名から打ち直しになった時点で応募をやめる。共通フォームコンポーネントに `input` の debounce 保存（`beforeunload` では間に合わない）と復帰時の自動流し込み、送信成功時のクリアを既定動作として内蔵する。氏名・電話を保持する以上、保存先は `localStorage` でなく `sessionStorage` に限定し、送信完了と離脱時に必ず消す
