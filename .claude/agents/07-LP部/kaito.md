...
### 2026-09-02
- **失敗パターン: 複製元 LP に埋まっていた GA4 測定 ID・GTM コンテナ ID・Meta ピクセルがコードに残ったまま公開され、クライアントの計測が 0 のまま／複製元企業の計測に流入データが送られる** → 回避策: `predeploy` の CI ゲート（2026-09-01参照）に `grep -rE "G-[A-Z0-9]{6,}|GTM-[A-Z0-9]+|fbq\('init'"` を追加し、クライアント指定 ID 以外を検出したら exit code で落とす。デプロイ後は GA4 のリアルタイムレポートで自分のアクセスが着弾するところまで確認し、フォームのダミー実送信（2026-08-05参照）と同じく「実データが正しい宛先に届いた」ことを納品完了の条件にする（理由: 計測タグは画面に現れないため、忠実度スコア満点でも最後まで誰も気づかない）
...
### 2026-09-09
- **失敗パターン: Vercel Cron Jobs をお知らせ自動更新等に設定する際 `VERCEL_ENV` 判定を入れず、Preview デプロイのたびに本番用 cron が誤発火し DB へ重複書き込みが発生する** → 回避策: cron 実行関数の先頭で `process.env.VERCEL_ENV === 'production'` を必須ガード化し、`vercel.json` の `crons` も Production プロジェクトにのみ紐付けてからデプロイする
...
### 2026-09-13
- **クライアント担当者は公開当日に自社名でスマホ検索し、LP が出てこないと「失敗した」と判断して連絡してくる**：昇格後に Search Console へインデックス登録をリクエストする運用（2026-09-02参照）は正しいが、担当者側には「公開＝検索に出る」という期待しかなく、インデックスまでの数日が事故として扱われる。納品連絡のテンプレ（2026-08-18参照）に「検索結果に反映されるまで数日かかること」「それまでは本番 URL 直打ちで確認すること」「求人票・名刺への URL 掲載は今日から可能なこと」の3行を常設し、期待値を送る側で先に揃える
