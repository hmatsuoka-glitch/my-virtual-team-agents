...
### 2026-08-03
- **ネイティブCSSネスティングと`@scope`が実装現場で定着**：ネスト記法とスコープ付きスタイルの普及で、STEP 1のCSS読み込みマップに「スコープ境界（`@scope (.card) to (.content)`）」を記録しないと、Renがグローバルセレクタで再現して意図せぬ他要素へ波及する。詳細度はネスト展開後で計算される点も併記する。
...
### 2026-08-12
- **（よくある失敗）疑似要素の`content`に入ったアイコンフォントのコードポイント（`\f001`等）や装飾記号を採り漏れ、アイコン・チェックマーク・引用符が複製版で消える**：::before/::afterの存在は拾えても`content`の中身を落とすと記号だけ消える。回避策：疑似要素抽出（2026-05-27参照）に`content`値とアイコンフォントのfont-family＋unicodeコードポイントをセット記録し、アイコンフォント依存箇所はlucide/heroicons等の代替（STEP 3）へのマッピングも添えてRenへ渡す。
- **（よくある失敗）`-webkit-line-clamp`（複数行の…省略）や`text-overflow: ellipsis`を採り漏れ、元は省略表示の長文が複製版で全表示になりカード高さ・レイアウトが崩れる**：省略指定は静的スクショの見た目には現れず落としやすい。回避策：STEP 4で行数省略（line-clamp）・1行省略（ellipsis）の指定と省略行数を記録し、Renへ「省略前提の設計」であることと、`above_fold_risk`（2026-06-07参照）計測時に省略後の高さで判定する旨を明記する。
- **（よくある失敗）半透明色（rgba/hsla のalpha・8桁HEX）を不透明HEXに丸めて採取し、オーバーレイやすりガラス上の文字背景が不透明になって下の画像が透けなくなる**：alphaを落とすと重なりの意図が壊れる。回避策：STEP 2でalphaチャンネルを持つ色は8桁HEX（`#RRGGBBAA`）またはrgba/`oklch(... / a)`で保持し丸め禁止、背景オーバーレイはstacking_map（2026-06-16参照）と合わせて透過の意図（下layerを透かす前提）を残す。backdrop-filter（2026-07-01参照）との併用箇所は特に注意。
- **（よくある失敗）SVGアイコンの`fill`/`stroke`が`currentColor`で親のcolorを継承している箇所を、抽出時点の見た目の色で固定してしまい、ホバー・ダークモード・テーマ切替でアイコン色が変わらなくなる**：見えている色で固定するとテーマ連動が死ぬ。回避策：STEP 5でSVGのfill/strokeが`currentColor`か固定値かを判別し、`currentColor`依存は「親color連動」フラグで記録して固定色化を禁止、CSS変数参照構造（2026-07-01参照）と同じくテーマ・状態連動を保持してRenへ渡す。
- **（よくある失敗）`@media (hover: none)`/`(pointer: coarse)`/`(orientation)`のインタラクション・入力デバイス別クエリを見落とし、タッチ端末やランドスケープで別挙動になる箇所を再現漏れする**：ビューポート幅のブレークポイントだけ見ると入力デバイス分岐が抜ける。回避策：STEP 6のメディアクエリ走査に`hover`/`pointer`/`orientation`を必ず含め、hover非対応環境の代替表示を`hover_only_content`フラグ（2026-06-07参照）と接続して記録し、タッチ端末で消えるUIを抽出段階で検出する。

### 2026-08-13
- **STEP抽出時の`tap_target_warning`・`hover_only_content`フラグ一覧を、ローンチ後にClarity分析するShunへ先渡しする**：Shunはヒートマップのデッドクリック（押せないのに押されている箇所）を「元設計由来か複製実装の差分か」で切り分けたい（Shun 2026-07-16参照）が、判定材料は自分の抽出フラグにある。抽出時に付ける操作性フラグ（2026-06-07参照）の該当箇所一覧をShunへ渡しておくと、Shunはフラグありなら「元設計由来」、フラグなしでデッドクリックなら「実装差分の疑い」と切り分けられ、LP部への差し戻しが的外れにならない。
...
### 2026-08-16
- **抽出値の「忠実再現」は、元サイトの可読性欠陥まで一緒に複製している自覚を持つ**：建設業の求職者は現場の休憩中、屋外の直射日光下＋画面反射のあるスマホでLPを見る。元サイトが薄いグレー本文（#999前後）や白背景×淡色見出しを使っていると、複製版は「元と同じ＝正しい」判定で通るのに、実際の閲覧環境では読めない。STEP 2のカラー抽出で本文・注釈のコントラスト比が4.5:1を割る箇所には`outdoor_readability_risk`フラグを立て、抽出値はそのまま渡しつつ「元サイト由来の欠陥」としてKaito経由でクライアントへ改善提案の余地を残す。忠実度と実用性を混ぜないための線引き。
...
### 2026-08-18
- STEP 2〜5の値取得はブラウザ上の手動確認でなく、computed style を一括ダンプする抽出スクリプトで機械取得し、人的判断はフラグ付けと例外処理だけに集中させる。目視転記は時間を食ううえ抽出漏れの主因でもある
...
### 2026-08-27
- **[更新] Iroとの着手前5分会は、役割分担とキー統一に加え「Iro側の正がロゴ色か実媒体色か」の確定状況まで議題にする（旧 2026-08-13 を更新）**：ブランド色＝Iro正／装飾色＝Hana正の切り分けと、`tokens.json`キーのIro`--brand-`接頭辞への完全一致・OKLCH色空間の統一は前提として維持する。そのうえでIro側は、ロゴ実体色と実媒体（作業着・社用車・現場看板）色にΔE00の乖離がある場合どちらを正とするかをSTEP 0でtsumugi経由に確定させる運用に変わった（Iro 2026-08-16参照）。5分会の議題を①役割分担 ②キー・色空間の統一 ③Iro側の正が確定済みか（未確定なら自分の抽出色は「暫定」ラベルでRenへ渡す）の3点固定にする。③を確認せず抽出色を確定として渡すと、後からIro版へ差し替わってTailwind `extend.colors`とトークン参照箇所の総入れ替えになる
...
### 2026-09-01
- **Nao向け・Ren向けの2ファイル生成はSTEP 8完了時のワンコマンドに固定し、納品直前の整形作業を発生させない**：出力を2系統に分ける方針（2026-08-27参照）は、分割を手作業でやると結局「納品前に切り出す時間」として戻ってくる。computed style一括ダンプ（2026-08-18参照）からセクション単位（Nao向け：構造・max-width・余白・subgrid判定）と要素単位（Ren向け：computed値＋フラグ＋px固定/相対）へ振り分ける変換を1コマンドにまとめ、STEP 8の完了処理として必ず両方が同時に出るようにする。片方だけ古い版で渡す事故も同時に消える
...
### 2026-09-02
- **（よくある失敗）`@media print`の指定を採り漏れ、印刷・PDF化された募集要項で背景色が飛んで白抜き文字が消える／リンク先URLが分からなくなる**：建設会社では担当者が募集要項を印刷して社内回覧し、求職者も要項をPDF保存するのに、抽出はビューポート表示のメディアクエリしか見ていない。回避策：STEP 6のメディアクエリ走査（2026-08-12参照のhover/pointer/orientation拡張と同じ列）に`print`を加え、`print-color-adjust`の指定・改ページ（`break-inside`/`break-after`）・印刷時に非表示化される要素を記録する。元サイトにprint指定が無い場合は「元サイト由来の欠落」としてKaito向けの改善提案リスト（2026-08-27参照）へ回し、忠実再現と実用性の線引き（2026-08-16参照）を崩さない
...
### 2026-09-09
- **（よくある失敗）`<template>`タグ・Web Componentsのシャドウ内部コンテンツをJSレンダリング完了前に抽出し、実際にユーザーへ表示される内容が仕様書から欠落する**：`<template>`要素はDOMに存在してもデフォルトでは非表示・非レンダリングで、JSがクローンして初めて実体化する。回避策はSTEP 1のプリフライト（2026-06-16参照）で`<template>`要素とカスタム要素の`.shadowRoot`（2026-05-20参照）の両方を走査し、JS実行完了後（`document.fonts.ready`と同様に描画安定を待機）の状態で再取得してから抽出を開始する。
...
### 2026-09-13
- **建設業の求職者は軍手・手袋のまま、あるいは手が汚れた状態でスマホを触るためタップ精度が落ち、隣接リンクの誤タップが離脱に直結する**：`tap_target_warning`（2026-08-18参照）はターゲット単体の寸法だけを見ているが、実際の誤タップは「寸法は足りているが隣と近すぎる」箇所で起きる。フラグの判定条件に「隣接するインタラクティブ要素との間隔が8px未満」を追加し、募集要項のリンク列・SNSアイコン並び・フッターナビを重点対象にする。元サイトが詰めている場合は値をそのまま渡したうえでKaito向け改善提案リスト（2026-08-27参照）へ回し、忠実再現と実用性の線引き（2026-08-16参照）は崩さない
