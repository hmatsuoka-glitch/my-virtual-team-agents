...
### 2026-08-05
- **失敗パターン: 再利用コンポーネント（Card等）を「今のLPの見た目」に最適化してprops設計し、特定セクション専用になって別セクションで再利用できず結局コピー実装される** → 回避策: 再利用部品のprops設計は現案件の具体値でなく「将来の可変軸（variant/slot）」で切り、2箇所以上での利用を前提にバリアントを定義する（2026-05-15の再利用2箇所ゲートの設計時失敗版）。単一箇所最適化で再利用性を殺さない
