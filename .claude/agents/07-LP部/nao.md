...
### 2026-07-01
- **失敗パターン: 設計書に Server/Client の境界を書かず、Ren がページ全体を `'use client'` にして SSR/SEO とバンドルを損なう** → 回避策: 各コンポーネントに「SC/CC 区分」と CC 化の根拠（状態・イベント・ブラウザAPI 使用箇所）を設計表へ明記し、CC 境界はインタラクションを持つ末端に限定して指定する（理由: 区分が無いと Ren は安全側で全 CC 化しがちで、TTI/INP 悪化と RSC の恩恵喪失を招く）
- **失敗パターン: props を「文字列を直接埋め込む」前提で設計し、リンク・改行・強調を含むコピーが入るとエスケープ崩れや `<br>` ベタ書きになる** → 回避策: テキスト props は「プレーン文字列／リッチ（部分強調・改行あり）」を型で区別し、リッチは構造化データ（配列やスロット）で受ける設計にする（理由: 後からコピーに強調やリンクが入ると、単純文字列 props では HTML 混入かエスケープ事故のどちらかになる）
...
### 2026-07-02
- **Hana の `tokens.json` キーとコンポーネント命名の1対1対応表を STEP 1 で明文化して Ren の命名揺れを潰す連携**：Hana の `color.primary` `font.heading.size` 等のキーと自分の `Hero` `CTAButton` 命名を対応表化し、Ren が「`tokens.color.primary` を `CTAButton.bg` にマッピング」と一発理解できるようにする。命名揺れ起因の Ren 質問ラリー5往復を0に。Hana の完成度が3点以下なら STEP 2 前に再抽出要求する。
...
### 2026-07-03
- **品質チェックポイント①「計測イベント設計表」を設計書の必須セクション化**：GA4 イベントを Ren 任せにすると `click_cta`/`ctaClick`/`button_click` と命名が割れ、計測が集計不能になる。STEP 5 で「イベント名（snake_case 統一）/発火条件（CTA クリック・フォーム送信・スクロール75%到達）/付与パラメータ/対応 `data-testid`」の 4 列表を設計書に定義し、Mia のフォーム E2E で GA4 発火確認までできる状態で Ren に渡す。納品後の『CV 計測できてない』発覚を設計層で封じる
...
### 2026-07-11
- **業界用語再確認「Atomic Design（atom/molecule/organism/template/page）」と機能単位設計の使い分け**：Atomic Design＝UI を原子→分子→有機体→テンプレ→ページの粒度で積む方法論。中小 LP では全面適用すると `atoms/` に断片が散り変更影響が追えなくなるため、共有 UI（Button/Input＝atom〜molecule）にのみ Atomic を適用し、セクション（Hero/Features＝organism 相当）は feature 単位でコロケーション（2026-06-13）する折衷が正しい。設計書に「どの粒度に Atomic を採るか」を明記し、方法論の全面/部分適用を判断軸として使う
...
### 2026-07-16
- **外部連携ありの案件は、システム開発部 Ao の Zod スキーマを STEP 3 前に受領して Form 仕様へ1対1で写す連携**：Ren が実装着手後にスキーマ照合すると `name`/`fullName` のフィールド名差やバリデーション差が判明して設計書ごと直しになる。tsumugi/Kaito 経由で Ao のスキーマを props 設計の前に取り寄せ、各フィールド行に「Ao 側キー名／必須任意／バリデーション／controlled・uncontrolled 区分」を並べて設計する。API 側が真実源である項目を設計層で確定させ、実装フェーズの照合を検算だけにする
...
### 2026-07-27
- **CSS Container Queries が全モダンブラウザで安定**：レスポンシブ設計を「画面幅ブレークポイント」から「コンポーネントが置かれた枠の幅」基準へ寄せられる。カード/サイドバー系の再利用性が上がるため、設計書に `@container` 前提の可変仕様を書くとページ配置に依存しない部品設計になる
...
### 2026-08-12
- **失敗パターン: z-index を各コンポーネントが場当たりの生数値で指定する設計にし、モーダルが固定ヘッダーの下に潜る・ドロップダウンがhero画像に隠れる重なり事故が起きる** → 回避策: z-indexは設計書で階層トークン（base=0 / header=100 / dropdown=200 / modal=1000 / toast=1100 等）として一元定義し、コンポーネント行には生数値でなくトークン名で指定。stacking context を生む `transform`/`opacity`/`position` の併用箇所も設計書に注記し、Renが場当たり値で重なり順を壊すのを設計層で封じる
- **失敗パターン: 画像・埋め込み動画・iframe・広告枠の寸法予約（width/height・aspect-ratio）を設計書に書かずRen任せにし、読み込み時のレイアウトシフト（CLS）が発生する** → 回避策: 全メディア要素にアスペクト比／予約高さを設計段階で明記し、CLSの責任を設計層で持つ（Kaito 2026-06-20の「CLSはNao設計領域」と対）。可変高さになりうる箇所（動的コンテンツ・広告）は `min-height` を設計値として与え、描画前後で高さが動かない状態を設計書の必須項目にする
- **失敗パターン: カード・リストを元LPの字数前提で固定高さ設計し、実データ（長い社名・多い実績件数・可変の職種数）で高さ崩壊・テキスト溢れが起きる** → 回避策: 可変コンテンツを持つコンポーネントは kotone の想定字数レンジ（2026-07-16参照）を各行に入れ、溢れ時挙動（`line-clamp`で省略か・高さ可変で許容か・スクロールか）を設計表で確定する。件数可変のグリッドは0件・1件・多件それぞれのレイアウト（空状態含む）を設計し、Ren/Miaが実データで崩れる事故を設計の受け口で防ぐ
- **失敗パターン: データ取得を伴うセクションのローディング/スケルトン状態を設計せず正常系だけ描き、fetch中の寸法未定義で表示時にガタつく／CLSが出る** → 回避策: fetchを伴うセクションはskeletonの寸法を実コンテンツと同寸で設計し、`Loading`状態を遷移フロー図（2026-05-15参照のエラー/ローディング必須化）に加えて寸法まで定義する。取得前後で高さが変わらない予約領域を設計値で与え、Renがスケルトンを後付けで場当たり実装するのを防ぐ
...
### 2026-08-16
- **セクション順を「会社が言いたい順」でなく「求職者が知りたい順」で設計し、並べ替えの根拠を設計書に明記する**：建設業の求職者が最初に確認するのは給与・休日・勤務地で、次に仕事内容、次に一緒に働く人、会社概要と代表挨拶は最後まで見られないか読み飛ばされる。一方クライアントの要望は会社概要・沿革・代表挨拶を上部に置く構成になりがちで、そのまま設計すると求職者の知りたい情報がスクロール3画面下に沈む。STEP 1 のセクション洗い出しに「求職者関心度」の列を足し、順序に手を入れた箇所は理由を書き添えて tsumugi 経由で説明できる状態にする。設計者の好みではなく閲覧者の関心順という根拠で並べ替えを通す
...
### 2026-08-27
- **[更新] Sota の意図的な崩しは `intentional: true` に加えて Mia の比較単位と同一のセクション ID を付けて設計書へ記す（旧 2026-08-13 を更新）**：注記を文章で書いても、Mia がセクション単位のベースライン比較（mia 2026-08-18参照）へ移行した後は「どの比較単位を除外すべきか」が機械的に決まらず、結局 QA 側の目視読み替えが残る。Sota から受けた非対称余白・定番崩しの箇所を、Mia が撮影単位に使うのと同じセクション ID へ紐づけて `intentional: true` と併記する。Mia は該当 ID のしきい値だけを緩められ、デザイン意図の伝達が QA の実行単位まで直結する
...
### 2026-09-01
- **[更新] 標準セクション順は1本でなく「未経験20代／経験者30代／事務・女性採用」の3ペルソナ分岐で持ち、案件では分岐を選んでから差分だけ設計する（旧 2026-08-18 を更新）**：求職者関心度順のテンプレを1本で運用すると、経験者向け案件では「条件先出し」より裁量・年収モデル・任される現場の順が効くため、毎回並べ替え議論が戻りテンプレを持つ意味が薄れる。kotone の訴求軸分岐（kotone 2026-06-24参照）と同じ3分岐でセクション順テンプレを持ち、tsumugi のヒアリングでペルソナが確定した瞬間に順序が決まる状態にする。案件で設計するのは分岐選択後の差分と、並べ替えた箇所の理由だけになる
...
### 2026-09-02
- **[更新] 再利用コンポーネントの props は「variant 3値まで・案件固有 boolean 禁止」を上限値として設計表に書く（旧 2026-08-05 を更新）**：現案件の具体値でなく将来の可変軸（variant/slot）で切る方針だけだと、実装が進むにつれ `isRecruitPage` `hasBadge` `compact` のような案件固有 boolean が足され、組み合わせ爆発でどの見た目が存在するのか誰も把握できなくなる。variant は列挙3値までを上限とし、4値目が必要になった時点で別コンポーネントへ分割、真偽で見た目が変わる指定は boolean でなく variant 名か `children` スロットで受ける、と props 列に上限を明記する。再利用2箇所以上のゲート（2026-05-15参照）を通った部品だけがこの上限の適用対象になる
...
### 2026-09-09
- **失敗パターン: 多言語対応の予定がないからと文言をコンポーネントに直接ベタ書きで設計し、後から英語版・多言語版の要望が来た際に全コンポーネントから文言を抽出し直す大改修になる** → 回避策: 多言語化予定の有無に関わらず、表示文言は必ず constants 経由で参照する設計を標準化し、i18n 化が必要になった時に constants ファイルの差し替えだけで対応できる状態を維持する
- **失敗パターン: フォーム送信完了画面を同一ページ内の state 切替（`useState`）だけで設計し、リロード・URL共有・ブラウザの戻る操作で完了状態が消えて再入力を求められる** → 回避策: 完了状態は専用ルート（`/contact/complete`）または URL クエリで表現する設計にし、リロード・共有・戻る操作への耐性を設計書のページ遷移フロー図に明記する
...
### 2026-09-13
- **[更新] 施工実績・お知らせ一覧は件数閾値の明記に留めず、「求職者は自分の通勤圏に現場があるかを探している」前提で初期6件＋エリア絞り込みを既定にする（旧 2026-09-09 を更新）**：全件一括DOM描画を避けるため件数閾値（20件超）と表示方式を設計書に書く原則は維持するが、既定を具体化しないと Ren は施工年度順のページネーションを選び、求職者にとっては通勤可否の判断材料にならない一覧が出来上がる。建設業の求職者が実績一覧で見ているのは会社の実力証明でなく「自宅から通える範囲の現場か」なので、初期表示はエリア別6件、絞り込みは市区郡→工種の2軸、絞り込み結果は URL クエリへ同期（2026-09-09参照）と設計表へ固定する。Ren の判断余地と求職者の探し直しを同時に消す
