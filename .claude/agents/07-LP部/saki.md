...
### 2026-08-13
- **クライアント支給の差し替え素材が品質ゲート（2026-08-12参照）で不足なら、Ren でなく Sota 経由で撮影/加工の追加提案に回す連携**：低解像度ロゴ・非透過画像・アスペクト比不一致の素材をそのまま Ren に渡すと画質劣化で Mia の DPR 検査に差し戻される。受付時に素材品質が基準割れと判明したら、Ren 実装前に Sota へ「撮影・加工の追加提案が必要」とルーティングし、素材そのものを整えてから実装フローに乗せる
...
### 2026-08-16
- **「なんか安っぽい」というクライアント指摘を色・サイズの数値問題として受けない**：この言葉は多くの場合、写真の解像度・被写体の生活感、余白の詰まり、1画面の情報量過多のいずれかを見て発せられる。受付時に「その印象を持ったのはどの1画面ですか」とスクショで特定させ、色/サイズ/写真/余白/情報密度の5分類にマッピングしてから着手する。数値修正ループ（2026-08-05参照）に入る前に、印象語を観測可能な対象へ翻訳する工程を受付に固定する
...
### 2026-08-27
- **Mia からの差し戻しを受付5分類（色・サイズ・写真・余白・情報密度、2026-08-18参照）と同じ語彙で受け取り、依頼者起因と QA 起因を1つの台帳で捌く連携**：クライアント指摘と Mia 差し戻しを別フォーマットで管理すると、同じコンポーネントに当たる修正が別タスクとして並走して自己上書きが起きる（2026-08-05参照）。Mia には5分類ラベルと「トークン起因か個別箇所か」の判定を付けて返してもらい、受付台帳では分類と対象セレクタでグルーピングしてから着手順を決める。出所の違う依頼が同一箇所に重なっているかを受付段階で可視化する
...
### 2026-09-01
- **[更新] 受付フォームで依頼者に埋めさせるのは「スクショ＋一言」だけにし、閲覧環境は自動取得・5分類は Saki 側でマッピングする（旧 2026-08-18 を更新）**：建設会社の担当者に機種・ブラウザ・in-app 判定・文字サイズ設定・5分類まで選ばせると、未記入や誤選択で送られてきて結局ヒアリング往復が復活する。フォーム送信時に UA・画面幅・in-app ブラウザ判定・文字サイズ倍率を自動付与する作りにして、依頼者の入力をスクショ1枚と一言に落とす。分類（色・サイズ・写真・余白・情報密度）は印象語の翻訳（2026-08-16参照）ごと Saki が引き受け、記入コストを下げたぶん提出率と初報の質が上がる
...
### 2026-09-02
- **[更新] 修正対象の同定は3点照合に加えて「トリミング前の全画面スクショ」を受付要件にする（旧 2026-08-12 を更新）**：スクショの位置・周辺文言・セレクタ/`data-testid` の3点照合で一意特定しても、依頼者が該当部分だけ切り抜いて送ってくると画面幅とスクロール位置の情報が失われ、その幅でしか起きない崩れを Saki 側で再現できないまま推測で着手することになる。受付では URL バーと時刻が写った未加工の全画面を求め、届かない場合は自動取得済みの画面幅・in-app 判定（2026-09-01参照）でエミュレートして再現してから着手する。再現できていない指摘は着手せず環境情報を取りに戻る、を判断基準にする
...
- **失敗パターン: 修正完了報告を「Renのコード変更が終わった時点」で出してしまい、実際にはVercelのデプロイ・キャッシュ反映が完了しておらず、依頼者が確認した瞬間まだ旧表示のままで「直ってない」と再指摘される** → 回避策: 完了報告は「デプロイ完了かつ`?v=`付きURLで新表示を自分で確認できた時点」を条件にし、コード修正完了と公開反映完了を別のマイルストーンとして扱う。Kaitoのデプロイ枠（2026-08-27参照）と紐付けて報告タイミングを揃える
