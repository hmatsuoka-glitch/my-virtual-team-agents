...
3. **制作実行**: 選んだタスクを Task tool で担当エージェント（subagent_type: kotone または sho）に委任して実行する
   ...
   - 集客コンテンツ → sho。X投稿は pilot-company/drafts/ に1ファイル1投稿。**note無料記事は pilot-company/tasks/note_queue/ に frontmatter付きで保存**（note-botが自動公開）。書式は note_queue/note_article_01.md を参照。**読まれる記事の必須ルール**:
     ...
     - 構成は逆三角形（結論→説明→補足）。本文200〜400字ごとに `##` 見出しで小グループ化。**本編の主要セクションは `## 1. …` `## 2. …` と番号付き見出しにする**（note-botが番号付き見出しの前に離脱防止バナー画像を自動挿入する）
     - スマホ可読性のため2〜3文ごとに改行（空行で段落を分ける）。**「しかし、」「そこで、」「つまり、」等の接続詞の直後で改行**して次への期待を作る（離脱防止）
     - 煽り・誇大表現・断定は禁止。事実と具体で書く（strategy/learnings準拠）
     - **★4 記事末尾に相互リンク行を必ず入れる**: 本文の最後に `―――` に続けて `Xでも発信中 → https://x.com/<現在のXハンドル>` を1行入れる（note→Xの動線。ハンドルは pilot-company/FUNNEL.md 参照）
   - **★2 動線ルール（X投稿→note記事）**: X投稿の内容に関連する公開済みnote記事がある場合、その下書きファイルの**末尾に `NOTE_REPLY: <note記事のフルURL>` を1行だけ**加える。X-botが本文を投稿した直後に、そのnoteリンクを**自己リプ**で貼りクリック導線を作る（本文にURLは直書きしない＝Xのリーチ低下を防ぐ）。URLが無い/未公開なら NOTE_REPLY 行は付けない
