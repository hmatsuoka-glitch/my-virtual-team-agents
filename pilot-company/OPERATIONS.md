...
- 全セッション共通ルール: publish 系の外部アクションは原則行わない。**唯一の例外はX投稿**（2026-07-07 HARU指示により自動化）。経路は次の2つのみ: ①`scripts/post_to_x.py`（API方式・本命） ②`browser/x_browser_bot.py`（ローカルボット・API移行までの暫定、2026-07-08 HARU了承）。いずれも日次上限2件・全件 logs/x_posts.csv に記録。停止はAPIキー削除／launchd unload。note公開・販売開始・支出は引き続き人間承認が必須
